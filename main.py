...
TOKEN = '8941581162:AAFO3WDkp0XM1YX7d7AGvj9wUoBOgWttflM'
...
def _resolve_api_route(product_name, plan_id=None, local_pid_id=None, duration=None):
    ...
    if cfg and cfg[1] and cfg[2]:
        return {'url':cfg[2],'api_key':cfg[1],'product_id':cfg[3] or str(local_pid_id or ''),'valid_id':_hack_server_plan_valid_id(product_name,plan_id) if plan_id else ''}
    return {'url':get_setting('server_api_url') or RESELLER_API_URL,'api_key':get_setting('server_api_key') or RESELLER_API_KEY,'product_id':str(local_pid_id or ''),'valid_id':''}
...
def fetch_key_from_api(pid_id, duration, android_id=None, product_name=None, plan_id=None):
    ...
    if not remote_product_id or remote_product_id=='0' or (str(remote_product_id).isdigit() and int(remote_product_id)<10): remote_product_id='133'
