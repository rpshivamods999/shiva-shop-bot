...
TOKEN = '8802969772:AAEzM65P0HN753CfS1lnVFA7KwPEdYO6NOg'

# Telegram Numeric Admin ID (Supports multiple admins if needed)
ADMIN_IDS = [8781522303, 6419247159]  
...
# FamGateway payment lifecycle settings.
# QR/payment verification is intentionally limited to 5 minutes.
...
def init_db():
    ...
    # Persistent gateway-order state. This supplements (not replaces) the
    # existing in-memory active_orders dictionary and records the payment
    # lifecycle for audit/exact-once verification.
    cursor.execute('''
...
def _inject_labeled_user_values(text, values):
    ...
    for labels, key in [
        ...
        (["Order ID"], "order_id"),
        (["Price Paid", "Unit Price", "Price", "Amount"], "amount"),
...
def _gateway_status_is_success(value):
    # FamGateway may represent success as a string or boolean/numeric value.
    if value is True:
        return True
    if isinstance(value, (int, float)) and value == 1:
        return True
    return str(value or '').strip().lower() in ('success', 'paid', 'credited')

def create_fam_qr(amount):
    # IMPORTANT: never generate a fake/local order id here. verify.php only knows
    # orders created by FamGateway, so a fallback QR would accept a payment that
    # this bot could never auto-verify.
    try:
        amount_value = round(float(amount), 2)
    except (TypeError, ValueError):
        print(f'[FamGateway] Invalid amount: {amount!r}')
        return None, None

    if amount_value <= 0:
        print(f'[FamGateway] Refusing invalid amount: {amount_value}')
        return None, None

    create_url = f"{BASE_GATEWAY_URL.rstrip('/')}/create_order.php"
    params = {'amount': f'{amount_value:.2f}', 'api_key': FAM_API_KEY}
    last_error = 'unknown gateway error'

    for attempt in range(1, 3):
        try:
            res = HTTP_SESSION.get(create_url, params=params, timeout=FAM_HTTP_TIMEOUT)
            res.raise_for_status()
            res_json = res.json()

            if _gateway_status_is_success(res_json.get('status')):
                data = res_json.get('data') or {}
                qr_url = data.get('qr_url')
                order_id = data.get('order_id')
                if qr_url and order_id:
                    print(f'[FamGateway] Order created: {order_id} amount=₹{amount_value:.2f}')
                    return str(qr_url), str(order_id)
                last_error = f'success response missing qr_url/order_id: {res_json}'
            else:
                last_error = str(res_json.get('message') or res_json)
    ...
    print(f'[FamGateway] Create-order failed after retries: {last_error}')
    return None, None

def verify_fam_order(order_id, expected_amount=None):
    if not order_id:
        return False, None, None

    verify_url = f"{BASE_GATEWAY_URL.rstrip('/')}/verify.php"
    params = {'order_id': str(order_id), 'api_key': FAM_API_KEY}

    try:
        res = HTTP_SESSION.get(verify_url, params=params, timeout=FAM_HTTP_TIMEOUT)
        res.raise_for_status()
        res_json = res.json()

        if not _gateway_status_is_success(res_json.get('status')):
            return False, None, None

        data = res_json.get('data') or {}

        # If the gateway supplies an amount, require it to match the Telegram
        # order. This prevents a successful UTR for a different amount from
        # completing this order.
        if expected_amount is not None and data.get('amount') not in (None, ''):
            try:
                gateway_amount = round(float(data.get('amount')), 2)
                wanted_amount = round(float(expected_amount), 2)
                if abs(gateway_amount - wanted_amount) > 0.01:
                    print(
                        f'[FamGateway] Amount mismatch for {order_id}: '
                        f'gateway=₹{gateway_amount:.2f}, expected=₹{wanted_amount:.2f}'
                    )
                    return False, None, None
            except (TypeError, ValueError):
                pass

        utr = data.get('utr') or data.get('UTR') or 'N/A'
        sender = data.get('sender_name') or data.get('sender') or 'UPI User'
        return True, str(utr), str(sender)

    except requests.RequestException as e:
        print(f'[FamGateway] Verify HTTP error for {order_id}: {e}')
    except ValueError as e:
        print(f'[FamGateway] Verify invalid JSON for {order_id}: {e}')
    except Exception as e:
        print(f'[FamGateway] Verify error for {order_id}: {e}')

    return False, None, None

def save_gateway_payment_order(order_id, user_id, amount, pay_type, product_id=0, device_id=None):
    # Persist the five-minute lifecycle of a gateway order.
    now = time.time()
    expires = now + FAM_PAYMENT_TIMEOUT
    conn = sqlite3.connect('shop_data.db', timeout=15)
    cur = conn.cursor()
    cur.execute('''
        INSERT INTO gateway_payment_orders
        (order_id, user_id, amount, pay_type, product_id, device_id, created_at, expires_at, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')
        ON CONFLICT(order_id) DO UPDATE SET
            user_id=excluded.user_id, amount=excluded.amount, pay_type=excluded.pay_type,
            product_id=excluded.product_id, device_id=excluded.device_id,
            created_at=excluded.created_at, expires_at=excluded.expires_at, status='pending',
            utr='', sender_name='', verified_at=0
    ''', (str(order_id), int(user_id), float(amount), str(pay_type), int(product_id or 0),
          str(device_id or ''), now, expires))
    conn.commit()
    conn.close()
    return expires

def mark_gateway_payment_verified(order_id, utr, sender_name):
    # Only one verifier can change a pending gateway order to paid.
    conn = sqlite3.connect('shop_data.db', timeout=15)
    cur = conn.cursor()
    now = time.time()
    cur.execute('''
        UPDATE gateway_payment_orders
        SET status='paid', utr=?, sender_name=?, verified_at=?
        WHERE order_id=? AND status='pending'
    ''', (str(utr or ''), str(sender_name or ''), now, str(order_id)))
    changed = cur.rowcount > 0
    conn.commit()
    conn.close()
    return changed

def mark_gateway_payment_expired(order_id):
    conn = sqlite3.connect('shop_data.db', timeout=15)
    cur = conn.cursor()
    cur.execute('''
        UPDATE gateway_payment_orders
...
def process_successful_payment(chat_id, user_id, message_id, order_id, amount, pay_type, utr, product_id=0, device_id=None):
    ...
    if pay_type == "topup":
        ...
        success_msg = (
            ...
            f"<i>Wallet Balance Added Successfully!</i>\n\n"
            ...
            f"├── 💵 <b>Amount Added:</b> ₹{amount:.2f} INR\n"
            f"├── 🏷️ <b>UTR:</b> <code>{html.escape(str(utr or 'N/A'))}</code>\n"
            f"└── 💼 <b>Current Balance:</b> ₹{new_bal:.2f} INR\n\n"
            f"✨ <i>Your wallet has been updated instantly.</i>"
        )
    ...
    bot.send_message(chat_id, success_msg, parse_mode="HTML", reply_markup=markup)

def poll_payment_in_bot(chat_id, user_id, message_id, order_id, amount, pay_type, product_id=0, device_id=None):
    # Poll FamGateway for at most exactly five minutes.
    deadline = time.monotonic() + FAM_PAYMENT_TIMEOUT
    ...
    while time.monotonic() < deadline:
        ...
        is_paid, utr, sender = verify_fam_order(order_id, expected_amount=amount)
        if is_paid:
            # Only the first verifier is allowed to complete the order.
            if mark_gateway_payment_verified(order_id, utr, sender):
                active_orders.pop(order_id, None)
                process_successful_payment(
                    chat_id, user_id, message_id, order_id, amount, pay_type,
                    utr, product_id, device_id
                )
            return

        # Never sleep beyond the five-minute deadline.
        time.sleep(min(FAM_POLL_INTERVAL, max(0.0, remaining)))

    if order_id in active_orders:
        active_orders.pop(order_id, None)
        mark_gateway_payment_expired(order_id)
...
def step_save_custom_text(message):
    ...
    if t_key == "top_up":
        # Add Balance is a shared presentation template only. The balance is
        # resolved live for each user when the screen is rendered.
        t_val = _repair_premium_emoji_markup(t_val)
...
def show_upi_checkout(chat_id, product_id, message_id=None, device_id=None):
    ...
    if not qr_url or not order_ref:
        error_text = (
            '❌ <b>Payment Gateway Temporarily Unavailable</b>\n\n'
            'The FamGateway order could not be created, so no payment QR was shown. '
            'Please try again in a few seconds. Your payment will never be marked '
            'successful without a real gateway order.'
        )
        bot.send_message(chat_id, error_text, parse_mode='HTML')
        return

    save_gateway_payment_order(
        order_ref, chat_id, final_price, 'purchase', product_id, device_id
    )
    ...
    if _has_custom_qr_payment_text():
        ...
        caption_parts.append(
            ...
            "└── 3️⃣ Pay within <b>5 minutes</b> and wait for auto-verification!"
        )
        caption_parts.append(
            "────────────────────────\n"
...
def show_topup_options(chat_id, message_id=None):
    user = get_user(chat_id)
    curr_bal = user[4] if user else 0.0

    default_topup_text = (
        f"💰 <b>Add Balance</b>\n"
        f"────────────────────────\n"
        f"💼 <b>Current balance:</b> ₹{curr_bal:.2f} INR\n"
        f"📥 <b>Min:</b> ₹1 INR  ·  📤 <b>Max:</b> ₹10,000 INR\n\n"
        f"<i>Pick an amount below or enter a custom amount.</i>"
    )

    # Presentation-only custom template. The account balance is NEVER saved in
    # this shared template; it is injected live for the current Telegram user.
    topup_template = get_custom_text("top_up", default_topup_text)
    topup_values = _get_user_template_values(chat_id, user[1] if user else None)
    topup_values.update({
        "user": topup_values.get("name", "User"),
        "first_name": topup_values.get("name", "User"),
        "balance": f"{float(curr_bal):.2f}",
    })
    topup_text = _safe_format_custom_text(topup_template, **topup_values)
    topup_text = _inject_labeled_user_values(
        topup_text,
        {
            "name": topup_values.get("name", "User"),
            "username": topup_values.get("username", ""),
            "username_at": topup_values.get("username_at", ""),
            "user_id": str(chat_id),
            "phone": topup_values.get("phone", "N/A"),
            "country": topup_values.get("country", "IN"),
            "balance": f"{float(curr_bal):.2f}",
            "ref_balance": topup_values.get("ref_balance", "0.00"),
            "orders": topup_values.get("orders", "0"),
            "lifetime": topup_values.get("lifetime", "0.00"),
            "deposited": topup_values.get("deposited", "0.00"),
            "joined": topup_values.get("joined", "N/A"),
        }
    )
    topup_text = _repair_premium_emoji_markup(topup_text).strip()

    markup = types.InlineKeyboardMarkup(row_width=2)
    btn_100 = make_ui_button("💰 ₹100 INR", callback_data="topup_amt_100", button_key="topup:100", default_color="green")
    btn_250 = make_ui_button("💰 ₹250 INR", callback_data="topup_amt_250", button_key="topup:250", default_color="green")
    btn_500 = make_ui_button("💰 ₹500 INR", callback_data="topup_amt_500", button_key="topup:500", default_color="green")
    btn_1000 = make_ui_button("💰 ₹1000 INR", callback_data="topup_amt_1000", button_key="topup:1000", default_color="green")
    btn_2000 = make_ui_button("💰 ₹2000 INR", callback_data="topup_amt_2000", button_key="topup:2000", default_color="green")
    btn_5000 = make_ui_button("💰 ₹5000 INR", callback_data="topup_amt_5000", button_key="topup:5000", default_color="green")
    btn_custom = make_ui_button("✏️ Custom Amount (INR)", callback_data="topup_custom", button_key="topup:custom", default_color="green")
    btn_back = make_ui_button("🔙 Back to Main", callback_data="back_to_main", button_key="nav:back_main", default_color="blue")

    markup.add(btn_100, btn_250)
    markup.add(btn_500, btn_1000)
    markup.add(btn_2000, btn_5000)
    markup.add(btn_custom)
    markup.add(btn_back)

    render_or_edit(chat_id, message_id, topup_text, markup)

def show_topup_confirm(chat_id, amount, message_id=None):
    user = get_user(chat_id)
    curr_bal = user[4] if user else 0.0
    after_deposit = curr_bal + amount

    confirm_text = (
        f"💸 <b>ADD BALANCE — Step 2 of 2</b>\n"
        f"────────────────────────\n"
        f"💼 <b>Current balance:</b> ₹{curr_bal:.2f} INR\n"
        f"💵 <b>Adding:</b> ₹{amount:.2f} INR\n"
        f"🆕 <b>After deposit:</b> ₹{after_deposit:.2f} INR\n\n"
        f"👇 <b>Choose a payment method</b>\n"
        f"<i>Tap the button below to proceed to payment.</i>"
    )

    markup = types.InlineKeyboardMarkup()
    btn_upi = make_ui_button(f"🇮🇳 UPI India — ₹{amount:.2f}", callback_data=f"pay_topup_upi_{amount}", button_key="payment:upi", default_color="blue")
    btn_back = make_ui_button("🔙 Back", callback_data="top_up", button_key="nav:back_topup", default_color="blue")

    markup.add(btn_upi)
    markup.add(btn_back)

    render_or_edit(chat_id, message_id, confirm_text, markup)
...
def show_topup_upi_screen(chat_id, amount, message_id=None):
    ...
    if not qr_url or not order_ref:
        error_text = (
            '❌ <b>Payment Gateway Temporarily Unavailable</b>\n\n'
            'The FamGateway order could not be created. Please try again in a few seconds.'
        )
        bot.send_message(chat_id, error_text, parse_mode='HTML')
        return

    save_gateway_payment_order(order_ref, chat_id, amount, 'topup', 0)

    caption_text = (
        f"🇮🇳 <b>ADD BALANCE — IN BOT SCAN & PAY</b>\n"
        f"────────────────────────\n\n"
        f"📦 <b>Deposit Details:</b>\n"
        f"├── 💰 <b>Amount Adding:</b> ₹{amount:.2f} INR\n"
        f"└── 🆔 <b>Order ID:</b> <code>{order_ref}</code>\n\n"
        f"📱 <b>How to Pay:</b>\n"
        f"├── 1️⃣ Scan the QR Code above using any UPI App\n"
        f"├── 2️⃣ Pay exact amount: <b>₹{amount:.2f}</b>\n"
        f"└── 3️⃣ Pay within <b>5 minutes</b> and wait for auto-verification!\n"
        f"────────────────────────\n"
        f"⚡ <i>Auto-checking live payment in background...</i>"
    )

    markup = types.InlineKeyboardMarkup()
    btn_back = make_ui_button("🔙 Back to Main", callback_data="back_from_qr", button_key="nav:back_qr", default_color="blue")
    markup.add(btn_back)
    ...
    threading.Thread(target=poll_payment_in_bot, args=(chat_id, chat_id, sent_msg.message_id, order_ref, amount, "topup", 0), daemon=True).start()
                ...
                if t_key == "qr_payment":
                    ...
                    if t_key == "top_up":
                        prompt = (
                            ...
                            "The original Add Balance text will stay the same. Send the same text/design with the Premium custom emoji(s) you want to add. "
                            "The balance is always live per user.\n\n"
                            "Use <code>{balance}</code> for the user's current balance if you want to place it manually. "
                            "The label <b>Current balance:</b> is also updated automatically.\n\n"
                            "Example live balance: <code>₹654.25 INR</code> for one user, <code>₹0.00 INR</code> for another."
                        )
                        msg = bot.send_message(call.message.chat.id, prompt, parse_mode="HTML")
                    elif t_key == "how_to_use":
