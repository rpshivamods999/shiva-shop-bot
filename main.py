...
TOKEN = '8839947716:AAEME-1-pSCq0db1YyxKdveX4Sg_MgL21kU'

# Telegram Numeric Admin ID (Supports multiple admins if needed)
DEFAULT_ADMIN_IDS = [8781522303, 6739795427]
...
# Admin IDs are persisted in the bot database so the owner can add/remove admins
# from Bot Settings without editing source code.
def _load_persisted_admin_ids():
    try:
        conn = sqlite3.connect('shop_data.db', timeout=15)
        row = conn.execute("SELECT value FROM settings WHERE key='admin_ids'").fetchone()
        conn.close()
        if row and row[0]:
            ids = [int(x) for x in json.loads(row[0]) if str(x).isdigit()]
            if ids:
...
def _persist_admin_ids():
    try:
        conn = sqlite3.connect('shop_data.db', timeout=15)
        conn.execute("INSERT INTO settings(key,value) VALUES('admin_ids',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value", (json.dumps(list(dict.fromkeys(ADMIN_IDS))),))
        conn.commit(); conn.close()
    except Exception:
        pass

def _register_next_step_handler(message, callback, *args, **kwargs):
    """Register a next-step handler that supports /cancel everywhere."""
    def _wrapped(next_message):
        uid = getattr(getattr(next_message, 'from_user', None), 'id', None)
        text = (getattr(next_message, 'text', None) or '').strip().lower()
        if uid is not None and text in ('/cancel', 'cancel'):
            admin_temp_data.pop(uid, None)
            try:
                bot.send_message(next_message.chat.id, '❌ <b>Command cancelled.</b>\n\nYou can return to the menu and start again.', parse_mode='HTML')
            except Exception:
                pass
            return
        return callback(next_message, *args, **kwargs)
    bot.register_next_step_handler(message, _wrapped)
...
def init_db():
    ...
    ''')
    try:
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_referral_codes_owner ON referral_codes(owner_user_id)")
    except Exception:
        pass
    try:
...
ADMIN_IDS[:] = _load_persisted_admin_ids()
_persist_admin_ids()

def get_fampay_api_key():
    return (get_setting('fampay_api_key') or FAM_API_KEY).strip()

def get_fampay_upi():
    return (get_setting('fampay_upi_id') or RECEIVER_UPI).strip()

def set_fampay_config(upi_id, api_key):
    set_setting('fampay_upi_id', str(upi_id or '').strip())
    set_setting('fampay_api_key', str(api_key or '').strip())

def get_paytm_upi():
    return (get_setting('paytm_upi_id') or '').strip()

def get_paytm_merchant_id():
    return (get_setting('paytm_merchant_id') or '').strip()

def set_paytm_config(upi_id, merchant_id):
    set_setting('paytm_upi_id', str(upi_id or '').strip())
    set_setting('paytm_merchant_id', str(merchant_id or '').strip())
...
def _render_live_price_template(template, live_rows, product_name=''):
    """Render an admin price-list design while preserving its visual structure.

    The saved message is presentation-only. The first complete Validity/Stock/Price
    sample is treated as the reusable plan block and is repeated for every live plan.
    Premium custom emojis, HTML formatting, title text, separators and other
    decorative content are kept intact.

    Supported dynamic fields:
      {days}, {validity}, {stock}, {stock_status}, {price}, {price_text}
    The same fields can also be written as labelled rows:
      Validity: ...
      Stock: ...
      Price: ...
    """
    ...
    def plain_line(line):
        value = re.sub(r'<tg-emoji\b[^>]*>.*?</tg-emoji>', '', str(line),
                       flags=re.I | re.S)
        value = re.sub(r'<[^>]+>', '', value)
        return html.unescape(value).strip()

    # Find the first complete dynamic sample block. The words themselves are
    # intentionally required so the renderer cannot accidentally overwrite a
    # decorative line containing only a number.
    first_validity = first_stock = first_price = None
    for i, line in enumerate(lines):
        plain = plain_line(line).lower()
    ...
    if first_validity is None or first_stock is None or first_price is None:
        # Keep backward compatibility with the older global renderer. If the
        # template explicitly contains {pricing_info}, the caller can still
        # inject the generated live block elsewhere.
        return text

    block_end = first_price
    header = lines[:first_validity]
    sample = lines[first_validity:block_end + 1]
    footer_lines = lines[block_end + 1:]

    # Remove copied/generated plan rows after the first sample. Keep genuine
    # footer text such as CHOOSE A PLAN and any custom notes/buttons.
    footer = []
    after_choose = False
    for line in footer_lines:
        ...
        is_compact_generated_line = (
            re.search(r'\b\d+\s*days?(?:\s*days)?\b', plain) and
            (any(mark in plain for mark in ('📊', '❌', '├', '└', 'stock', 'price'))
             or after_choose)
        )
        if is_labeled_plan_line or is_compact_generated_line:
            continue
        footer.append(line)

    # The exact legacy heading is generated by the old renderer. Do not duplicate
    # it when an admin pasted an old price list into the new editor.
    header = [
        line for line in header
        if plain_line(line).lower() not in (
            'stock & pricing :', 'stock & pricing:', 'stock & pricing'
        )
    ]

    def replace_label_value(line, label, value):
        """Replace only the value after a label's colon.

        This deliberately keeps everything before the colon untouched, including
        Premium <tg-emoji> wrappers, bold/italic tags, tree characters and spaces.
        """
        escaped_value = str(value)
        # The value may be wrapped by an HTML tag, e.g. <b>Stock:</b>.
        # Replace visible text only and keep the closing/opening formatting tags.
        pat = re.compile(
            r'(\b' + re.escape(label) + r'\b[^:\n]*[:：])([ \t]*)([^<\n]*)',
            re.I
        )
        if pat.search(line):
            return pat.sub(lambda m: m.group(1) + m.group(2) + escaped_value, line, count=1)
        return line

    def replace_dynamic_day_line(line, days, validity):
        """Support the screenshot-style day line in addition to Validity:."""
        out = line
        # Explicit placeholders always win.
        out = out.replace('{days}', days).replace('{validity}', validity)
        # If a line says "Validity" without a colon, still update a numeric
        # sample value such as "Validity 1 DAYS".
        if re.search(r'\bvalidity\b', plain_line(out), re.I):
            out = re.sub(
                r'(\bvalidity\b[^0-9\n]*?)\b\d+\s*days?\b',
                lambda m: m.group(1) + validity,
                out,
                count=1,
                flags=re.I
            )
    ...
    for row in live_rows:
        ...
        stock = str(row.get('stock_status', 'Out of Stock') or 'Out of Stock')
        price = str(row.get('price_text', '') or '')
        ...
        discount_text = f'{discount_percent:.0f}% Off' if discount_percent > 0 else ''
        vals = {
            'days': days,
            'validity': validity,
            'stock': stock,
            ...
            'hack': str(product_name or ''),
        }

        out_lines = []
        for line in sample:
            line2 = replace_dynamic_day_line(line, days, validity)
            line2 = replace_label_value(line2, 'Validity', validity)
            line2 = replace_label_value(line2, 'Stock', stock)
            line2 = replace_label_value(line2, 'Price', price)

            # Replace placeholders without disturbing any Premium emoji HTML.
            for key, value in vals.items():
                line2 = line2.replace('{{' + key + '}}', str(value))
                line2 = line2.replace('{' + key + '}', str(value))
...
def _render_plan_text_product(template, live_rows, product_name):
    """Render a per-product Premium design with live plans/stock/prices."""
    return _render_live_price_template(template, live_rows, product_name)
...
def _render_hack_price_ui_template(template, pricing_rows, product_name):
    ...
    Both the instruction/sample style and the final compact Telegram style are
    supported. The admin supplies ONE Validity/Stock/Price sample; live plans are
    repeated automatically and all Premium custom emojis/formatting remain in
    their original positions.
    """
    return _render_live_price_template(template, pricing_rows, product_name)
...
def is_user_banned(user_id):
    try:
        conn = sqlite3.connect('shop_data.db', timeout=15)
        row = conn.execute('SELECT COALESCE(is_banned,0) FROM users WHERE user_id=?', (int(user_id),)).fetchone()
        conn.close()
...
def _generate_referral_code():
    ...
    while True:
        code='REF-' + ''.join(random.choice(alphabet) for _ in range(6))
        conn=sqlite3.connect('shop_data.db',timeout=15)
...
def get_user_referral_code(user_id):
    conn=sqlite3.connect('shop_data.db',timeout=15)
    row=conn.execute('SELECT referral_code FROM users WHERE user_id=?',(int(user_id),)).fetchone()
    conn.close()
    return (row[0] or '') if row else ''

def create_or_refresh_user_referral_code(user_id, refresh=False):
    existing=get_user_referral_code(user_id)
    if existing and not refresh: return existing
    code=_generate_referral_code()
    conn=sqlite3.connect('shop_data.db',timeout=15)
    conn.execute('UPDATE users SET referral_code=? WHERE user_id=?',(code,int(user_id)))
    conn.execute('INSERT OR REPLACE INTO referral_codes(code,owner_user_id,created_at) VALUES(?,?,?)',(code,int(user_id),datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
    conn.commit(); conn.close()
    return code
...
def get_referral_code_owner(code):
    ...
    conn=sqlite3.connect('shop_data.db',timeout=15)
    row=conn.execute('SELECT owner_user_id FROM referral_codes WHERE code=?',(code,)).fetchone()
    conn.close()
...
def refund_user_balance(user_id, amount):
    """Refund a failed wallet checkout without changing total_deposited."""
    conn = sqlite3.connect('shop_data.db', timeout=15)
    try:
        cursor = conn.cursor()
        cursor.execute('UPDATE users SET balance = balance + ? WHERE user_id = ?', (float(amount), int(user_id)))
        conn.commit()
    finally:
        conn.close()
...
def rename_product_db(old_name, new_name):
    old_name=str(old_name or '').strip(); new_name=str(new_name or '').strip()
    if not old_name or not new_name: return False, 'empty'
    if old_name == new_name: return True, 'same'
    conn=sqlite3.connect('shop_data.db',timeout=15)
    exists=conn.execute('SELECT 1 FROM products WHERE name=? LIMIT 1',(new_name,)).fetchone()
    if exists:
        conn.close(); return False, 'exists'
    cur=conn.execute('UPDATE products SET name=? WHERE name=?',(new_name,old_name))
    conn.commit(); conn.close()
...
def _extract_api_key_value(data):
    """Extract a real delivered key from common REST/JSON response shapes.

    Providers do not always return the key in exactly the same envelope.  Some
    responses use key/license fields directly, while others put a key list or
    nested delivery object inside data/result/order.  This extractor deliberately
    follows only key-related fields and known response containers so that a
    product name, status message, price, or order id is never mistaken for a key.
    """
    direct_fields = (
        'key', 'keys', 'license', 'license_key', 'licenseKey', 'license_keys',
        'licenseKeys', 'licence', 'licences', 'serial', 'serial_key',
        'code', 'codes', 'key_value', 'keyValue', 'access_key', 'accessKey',
        'product_key', 'productKey', 'generated_key', 'generatedKey',
        'delivery_key', 'deliveryKey', 'key_data', 'keyData', 'key_value_data'
    )
    nested_fields = (
        'data', 'result', 'response', 'order', 'delivery', 'sale', 'purchase',
        'license_data', 'licenseData', 'delivery_data', 'deliveryData',
        'key_data', 'keyData', 'details', 'payload'
    )

    if isinstance(data, dict):
        # First preference: explicitly key-named fields.
        for field in direct_fields:
            ...
            if isinstance(value, str):
                text = value.strip()
                if len(text) > 3 and text.lower() not in {'success', 'true', 'false', 'ok', 'pending'}:
                    return text
                ...
                for item in value:
                    if isinstance(item, str):
                        ...
                        found = _extract_api_key_value(item)
                        if found:
                            return found
            elif isinstance(value, dict):
                found = _extract_api_key_value(value)
                if found:
                    return found

        # Then follow only documented/common response containers.
        for field in nested_fields:
            if field in data:
                found = _extract_api_key_value(data.get(field))
                if found:
                    return found
...
def _looks_like_provider_order_id(value):
    value=str(value or '').strip()
    if len(value)<4:
        return False
    upper=value.upper()
    if upper.startswith(('ORD','ORDER','SALE','PUR','TXN','RAP','API')):
        return True
    return len(value)>=8 and bool(re.search(r'[A-Za-z]',value))


def _extract_api_order_id(data):
    """Extract provider order/sale IDs, including nested order.id shapes."""
    if isinstance(data,dict):
        for field in ('order_id','orderId','purchase_id','purchaseId','sale_id','saleId','transaction_id','transactionId','order_number','orderNo','reference','ref'):
            value=data.get(field)
            if value not in (None,'') and not isinstance(value,(dict,list)) and _looks_like_provider_order_id(value):
                return str(value).strip()
        for field in ('order','sale','purchase','transaction','data','result','response','delivery'):
            value=data.get(field)
            if isinstance(value,str) and _looks_like_provider_order_id(value):
                return value.strip()
            found=_extract_api_order_id(value)
            if found:
                return found
        value=data.get('id')
        if value not in (None,'') and not isinstance(value,(dict,list)) and _looks_like_provider_order_id(value):
...
def _extract_api_identifiers_from_raw(raw):
    """Recover key/order fields from malformed or non-standard JSON text."""
    raw=str(raw or '')
    key=None; order=None
    for pattern in (
        r'"(?:key|license_key|licenseKey|licence|serial|product_key|productKey|generated_key|generatedKey|access_key|accessKey)"\s*:\s*"([^"]+)"',
        r"'(?:key|license_key|licenseKey|licence|serial|product_key|productKey|generated_key|generatedKey|access_key|accessKey)'\s*:\s*'([^']+)'",
    ):
        m=re.search(pattern,raw,re.I)
        if m and len(m.group(1).strip())>3:
            key=m.group(1).strip(); break
    for pattern in (
        r'"(?:order_id|orderId|purchase_id|purchaseId|sale_id|saleId|transaction_id|transactionId|reference|ref|order)"\s*:\s*"?([A-Za-z0-9_-]{4,})',
        r'\b(ORD[A-Za-z0-9_-]{5,})\b',
    ):
        m=re.search(pattern,raw,re.I)
        if m and _looks_like_provider_order_id(m.group(1)):
            order=m.group(1).strip(); break
    return key,order


def _new_api_get_order_key(server_url, api_key, order_id, max_wait_seconds=300):
    """GET-only recovery for an already-created provider order."""
    server_url=_normalize_api_server_url(server_url); api_key=str(api_key or '').strip(); order_id=str(order_id or '').strip()
    if not server_url or not api_key or not order_id:
        return None
    headers={'Authorization':f'Bearer {api_key}','Accept':'application/json','User-Agent':'RG-Cheat-Shop-Bot/1.3'}
    encoded=urllib.parse.quote(order_id,safe='')
    endpoints=(
        f'{server_url}/api/v1/orders/{encoded}',
        f'{server_url}/api/v1/order/{encoded}',
        f'{server_url}/api/v1/orders?order_id={encoded}',
        f'{server_url}/api/v1/orders?id={encoded}',
        f'{server_url}/api/v1/order?id={encoded}',
        f'{server_url}/api/v1/orders/{encoded}/key',
        f'{server_url}/api/v1/order/{encoded}/key',
    )
    deadline=time.time()+max(1,int(max_wait_seconds or 300)); last_detail=''
    while time.time()<=deadline:
        for endpoint in endpoints:
            try:
                response=HTTP_SESSION.get(endpoint,headers=headers,timeout=5)
                raw=response.text.strip(); data=_api_server_parse_json(response)
                key=_extract_api_key_value(data); raw_key,_=_extract_api_identifiers_from_raw(raw); key=key or raw_key
                print(f'DEBUG NEW API ORDER LOOKUP status={response.status_code} order={order_id} url={endpoint} raw={raw[:1200]}')
                if response.status_code==200 and key:
                    return key
                last_detail=f'HTTP {response.status_code}: {raw[:300]}'
        ...
        time.sleep(1.0)
    print(f'DEBUG NEW API ORDER LOOKUP FAILED order={order_id}: {last_detail}')
    return None


def _new_api_generate_key(server_url, api_key, product_id, valid_id, duration=None, android_id=None, purchase_ref=None):
    """Create a provider sale safely and recover its key without duplicate POSTs.

    Validation-only 4xx responses may try the server's supported field aliases.
    Once any request returns 2xx/409, or the network result is ambiguous, the
    function stops immediately: it will never submit another purchase payload.
    """
    server_url=_normalize_api_server_url(server_url); api_key=str(api_key or '').strip()
    product_id=str(product_id or '').strip(); valid_id=str(valid_id or '').strip()
    android_id=str(android_id or '').strip(); duration=str(duration or '').strip(); purchase_ref=str(purchase_ref or '').strip()
    if not server_url or not api_key or not product_id or not valid_id:
        print('DEBUG NEW API: missing server/product/valid mapping'); return None
    if not purchase_ref:
        purchase_ref=f'{time.time_ns()}-{random.randint(100000,999999)}'

    idem=hashlib.sha256('|'.join((server_url,product_id,valid_id,android_id,duration,purchase_ref)).encode('utf-8')).hexdigest()[:48]
    headers={'Authorization':f'Bearer {api_key}','Accept':'application/json','Content-Type':'application/json','Idempotency-Key':idem,'User-Agent':'RG-Cheat-Shop-Bot/1.3'}

    # Keep compatibility with servers that use valid_id, variant_id or
    # validity_id, but only move to another alias after a validation 4xx.
    payloads=[]
    for alias in ('valid_id','variant_id','validity_id'):
        base={'product_id':product_id,alias:valid_id}
        if duration: payloads.append({**base,'duration':duration})
        if android_id: payloads.append({**base,'android_id':android_id})
    ...
    for payload in payloads:
        ...
        seen.add(marker)
        try:
            response=HTTP_SESSION.post(f'{server_url}/api/v1/generate-key',json=payload,headers=headers,timeout=20)
            raw=response.text.strip(); data=_api_server_parse_json(response)
            key=_extract_api_key_value(data); order_id=_extract_api_order_id(data)
            raw_key,raw_order=_extract_api_identifiers_from_raw(raw); key=key or raw_key; order_id=order_id or raw_order
            print(f'DEBUG NEW API GENERATE-KEY payload={marker} status={response.status_code}: {raw[:2000]}')

            if response.status_code in (200,201,202,409) and key:
                print('DEBUG NEW API GENERATE-KEY SUCCESS: key returned directly')
                return key
            if response.status_code in (200,201,202,409) and order_id:
                print(f'DEBUG NEW API ORDER CREATED {order_id}; GET-only recovery')
                key=_new_api_get_order_key(server_url,api_key,order_id,max_wait_seconds=300)
                if key: return key
                print(f'DEBUG NEW API: order {order_id} exists but key is not ready; no second purchase')
                return None
            if response.status_code in (200,201,202):
                print('DEBUG NEW API: successful response had no parseable key/order; retry disabled to prevent duplicate charge')
                return None

            # Only validation/not-found 4xx responses are candidates for the
            # next alias. Authentication, rate-limit and server errors stop.
            if response.status_code in (401,403,429) or response.status_code >= 500:
                return None
            if not (400 <= response.status_code < 500):
                return None
        except Exception as exc:
            # A timeout after POST is ambiguous: the provider may have charged.
            # Never retry automatically in that case.
            print(f'DEBUG NEW API GENERATE-KEY EXCEPTION: {str(exc)[:500]}')
            return None

    print('DEBUG NEW API GENERATE-KEY FAILED: all validation payload variants rejected')
    return None


def fetch_key_from_api(pid_id, duration, android_id=None, product_name=None, plan_id=None, purchase_ref=None):
    route = _resolve_api_route(product_name or '', plan_id, pid_id, duration)
    curr_api_url = route['url']
    curr_api_key = route['api_key']
    remote_product_id = route['product_id']
    valid_id = route.get('valid_id', '')

    if not remote_product_id or remote_product_id == '0':
        remote_product_id = '133'
    if not duration or duration == '0':
        duration = '1 Hours'

    # NEW REST v1 servers from the supplied integration guide use Bearer auth and
    # POST /api/v1/generate-key. Product ID + per-plan Valid ID are mandatory.
    if route.get('server_type') in ('new', 'new_multi'):
        key_value = _new_api_generate_key(
            curr_api_url,
            curr_api_key,
            remote_product_id,
            valid_id,
        ...
        if key_value:
            return key_value
        # Do not send a v1 purchase to the legacy root after a failed v1 purchase.
        # Doing so can create duplicate/incorrect orders or hide a real API error.
        return None

    # Existing reseller/PHP contract is preserved exactly for legacy routes.
    headers = {
    ...
    payload = {
        ...
        'action': 'buy',
        'product_id': str(remote_product_id).strip(),
        'duration': str(duration).strip()
    }
    if valid_id:
        payload.update({'valid_id': str(valid_id).strip(), 'validity_id': str(valid_id).strip()})
    if android_id:
        payload['android_id'] = str(android_id).strip()
    try:
        response = HTTP_SESSION.post(curr_api_url, data=payload, headers=headers, timeout=20)
        raw_resp = response.text.strip()
        print(f'DEBUG PROVIDER API RESPONSE: {raw_resp}')
        data = _api_server_parse_json(response)
        key_value = _extract_api_key_value(data)
        if key_value:
            return key_value
        if raw_resp and not raw_resp.startswith('{') and not raw_resp.startswith('[') and len(raw_resp) < 120:
            return raw_resp
...
def notify_admins_purchase(user_id, u_name, order_id, prod_name, prod_days, amount,
                           device_id=None, pay_type="upi", purchased_key=None):
    """Notify every admin after a successful product purchase.

    Includes the buyer identity, Telegram ID, product/plan, delivered key,
    payment method and exact purchase time. Sending is deliberately isolated
    from checkout so a notification failure can never break delivery.
    """
    user = get_user(user_id)
    phone = user[2] if user and len(user) > 2 else "N/A"
    country = user[3] if user and len(user) > 3 else "IN"
    dev_str = f"\n📱 <b>Device ID:</b> <code>{html.escape(str(device_id))}</code>" if device_id else ""
    payment_method = "💼 Wallet Balance" if str(pay_type).lower() == "wallet" else "🇮🇳 UPI Payment"
    purchase_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    key_text = str(purchased_key or "").strip()
    default_purchase_text = (
        f"🚨 <b>NEW HACK PURCHASE NOTIFICATION!</b> 🚨\n\n"
        f"👤 <b>User:</b> {html.escape(str(u_name or 'User'))}\n"
        f"🆔 <b>Telegram ID:</b> <code>{user_id}</code>\n"
        f"📱 <b>Phone:</b> <code>{html.escape(str(phone or 'N/A'))}</code>\n"
        f"🌐 <b>Country:</b> {html.escape(str(country or 'IN'))}\n"
        f"🎮 <b>Hack Purchased:</b> {html.escape(str(prod_name))}\n"
        f"⏱️ <b>Plan:</b> {html.escape(str(prod_days))}\n"
        f"💰 <b>Price Paid:</b> ₹{amount:.2f} INR\n"
        f"💳 <b>Payment Method:</b> {payment_method}\n"
        f"🔑 <b>Delivered Key:</b> <code>{html.escape(key_text or 'N/A')}</code>\n"
        f"🆔 <b>Order ID:</b> <code>{html.escape(str(order_id))}</code>{dev_str}\n"
        f"📅 <b>Date/Time:</b> {purchase_time}"
    )
    live_username = _get_live_telegram_username(user_id, u_name)
    ...
    # If a saved custom template omitted a key/method label, append the two
    # requested transaction facts without changing the rest of its design.
    plain_admin = re.sub(r"<[^>]+>", "", admin_purchase_text)
    if "Payment Method" not in plain_admin and "Method" not in plain_admin:
        admin_purchase_text += f"\n💳 <b>Payment Method:</b> {payment_method}"
    if key_text and "Delivered Key" not in plain_admin and "YOUR KEY" not in plain_admin and "Key:" not in plain_admin:
        admin_purchase_text += f"\n🔑 <b>Delivered Key:</b> <code>{html.escape(key_text)}</code>"
    admin_purchase_text = _repair_telegram_html_markup(_repair_premium_emoji_markup(admin_purchase_text))
    for admin_id in ADMIN_IDS:
        try:
            bot.send_message(admin_id, admin_purchase_text, parse_mode="HTML")
        except Exception:
            try:
                plain = re.sub(r"<tg-emoji\b[^>]*>(.*?)</tg-emoji>", lambda m: m.group(1), admin_purchase_text, flags=re.I | re.S)
                plain = re.sub(r"<[^>]+>", "", plain)
                bot.send_message(admin_id, html.unescape(plain))
            except Exception:
                pass
...
def _extract_gateway_qr_payload(data, qr_url, order_id, amount):
    ...
    return (
        f"upi://pay?pa={urllib.parse.quote(get_fampay_upi(), safe='@')}"
...
def create_fam_qr(amount):
    ...
    try:
        res = HTTP_SESSION.get(
            create_url,
            params={"amount": f"{amount:.2f}", "api_key": get_fampay_api_key()},
            timeout=10,
        )
        res.raise_for_status()
        res_json = res.json()
...
def verify_fam_order(order_id):
    verify_url = f"{BASE_GATEWAY_URL}/verify.php?order_id={order_id}&api_key={get_fampay_api_key()}"
    try:
...
def process_successful_payment(chat_id, user_id, message_id, order_id, amount, pay_type, utr, product_id=0, device_id=None):
    ...
    if pay_type == "topup":
        ...
        if not key_val:
            key_val = fetch_key_from_api(pid_id, remote_dur, android_id=device_id, product_name=prod_name, plan_id=product[0], purchase_ref=order_id)
        if not key_val:
            bot.send_message(
                chat_id,
                "❌ <b>Payment verified, but key delivery failed.</b>\n\n"
                "The API server did not return a key for the configured Product ID / Valid ID. "
                "Your payment was verified; please contact Admin so the order can be completed manually.",
                parse_mode="HTML"
            )
            return

        record_order(order_id, user_id, prod_name, prod_days, amount, key_val)
        notify_admins_purchase(user_id, u_name, order_id, prod_name, prod_days, amount, device_id, pay_type, key_val)
...
DEFAULT_WELCOME_MESSAGE='👋 <b>Welcome to RG CHEAT SHOP!</b>\n\n<i>Premium keys • Instant delivery • Secure checkout</i>'
def get_welcome_message_template(): return get_setting(WELCOME_MESSAGE_KEY) or DEFAULT_WELCOME_MESSAGE
def set_welcome_message_template(template): set_setting(WELCOME_MESSAGE_KEY,_repair_premium_emoji_markup(str(template or '').strip()))
def _render_welcome_message(user_id,first_name):
    values=_get_user_template_values(user_id,first_name); values.update({'user':values['name'],'first_name':values['name'],'telegram_id':str(user_id)})
    text=_safe_format_custom_text(get_welcome_message_template(),**values); text=_inject_labeled_user_values(text,values); return _repair_telegram_html_markup(_repair_premium_emoji_markup(_replace_live_greeting_name(text,first_name)).strip())
def _delete_message_safely(chat_id,message_id):
    if message_id:
        try: bot.delete_message(chat_id,message_id)
        except Exception: pass
def _send_welcome_then_main(chat_id,user_id,first_name,start_message_id=None):
    welcome_msg=None
    try: welcome_msg=bot.send_message(chat_id,_render_welcome_message(user_id,first_name),parse_mode='HTML')
    except Exception: pass
    try: show_shop_features(chat_id,first_name)
    finally:
        if welcome_msg: _delete_message_safely(chat_id,welcome_msg.message_id)
        _delete_message_safely(chat_id,start_message_id)

# ==================== BOT COMMANDS & HANDLERS ====================

@bot.message_handler(func=lambda message: bool(getattr(getattr(message, 'from_user', None), 'id', None)) and getattr(message.from_user, 'id', None) not in ADMIN_IDS and is_user_banned(message.from_user.id))
def _blocked_banned_user(message):
    try:
        bot.send_message(message.chat.id, '🚫 <b>Your account is currently banned.</b>\n\nPlease contact the administrator if you believe this is a mistake.', parse_mode='HTML')
    except Exception:
        pass
...
def send_welcome(message):
    ...
    if not user or user[10] == 0:
        ...
        try:
            welcome_msg = bot.send_message(message.chat.id, _render_welcome_message(user_id, first_name), parse_mode='HTML')
            admin_temp_data.setdefault(user_id, {})['welcome_message_id'] = welcome_msg.message_id
        except Exception:
            pass
        ...
        _delete_message_safely(message.chat.id, message.message_id)
    else:
        _send_welcome_then_main(message.chat.id, user_id, first_name, start_message_id=message.message_id)
...
def handle_contact_verification(message):
    if message.contact is not None:
        ...
        was_already_verified = bool(user and len(user) > 10 and user[10] == 1)
        welcome_message_id = admin_temp_data.get(user_id, {}).get('welcome_message_id')
        ...
        # Only a first successful verification is a "New User" event.
        if not was_already_verified:
            for admin_id in ADMIN_IDS:
                try:
                    ...
                    try:
                        plain = re.sub(r"<tg-emoji\b[^>]*>(.*?)</tg-emoji>", lambda m: m.group(1), admin_notification_text, flags=re.I | re.S)
                        plain = re.sub(r"<[^>]+>", "", plain)
                        bot.send_message(admin_id, html.unescape(plain))
                    except Exception:
                        pass
        ...
        _delete_message_safely(message.chat.id, welcome_message_id)
        admin_temp_data.pop(user_id, None)
        _send_welcome_then_main(message.chat.id, user_id, first_name, start_message_id=message.message_id)
...
# ==================== LEGACY ADMIN PANEL SOURCE (PRESERVED, NOT EXECUTED) ====================
# The original admin panel source is retained verbatim below so existing labels/text
# are not discarded while the live menu is organized into categories.
# @bot.message_handler(commands=['admin'])
def admin_panel(message):
    user_id=message.from_user.id
    if user_id not in ADMIN_IDS:
        bot.send_message(message.chat.id,'❌ <b>Access Denied!</b> Only Admin can access this command.',parse_mode='HTML'); return
    markup=types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton('⚙️ All Settings',callback_data='admin_cat_setup'))
    markup.add(types.InlineKeyboardButton('🌐 API & Server Systems',callback_data='admin_cat_api'))
    markup.add(types.InlineKeyboardButton('👤 User System',callback_data='admin_cat_users'))
    markup.add(types.InlineKeyboardButton('💳 Bot Payment System',callback_data='admin_cat_payment'))
    markup.add(types.InlineKeyboardButton('🛠️ Bot Settings',callback_data='admin_cat_bot_settings'))
    bot.send_message(message.chat.id,'🛠️ <b>ADMIN CONTROL PANEL</b>\n\nChoose a setting below.',parse_mode='HTML',reply_markup=markup)

def _admin_category_markup(category):
    groups={
      'setup':[("⚙️ All Settings","all_settings_open")],
      'api':[("🆕 New Server API System","new_server_api")],
      'users':[("👤 User Management","user_system_open")],
      'payment':[("💳 FamPay Auto Payment","payment_fampay"),("🇮🇳 Paytm UPI Auto Payment","payment_paytm")],
      'bot_settings':[("✏️ Bot Text & Premium Emoji","bot_settings_texts"),("🎨 Bot Button Color","bot_settings_colors"),("✨ Bot Button Emoji","bot_settings_emojis"),("💾 Bot Backup","bot_settings_backup"),("👑 Admin Management","bot_settings_admins")]
    }
    markup=types.InlineKeyboardMarkup()
    for label,cb in groups.get(category,[]): markup.add(types.InlineKeyboardButton(label,callback_data=cb))
    markup.add(types.InlineKeyboardButton('🔙 Main Admin Panel',callback_data='admin_main'))
    return markup
...
def step_process_user_reset(message):
    try:
        ...
        _register_next_step_handler(msg, step_process_user_reset)
...
def step_edit_product_price(message):
    ...
    try:
        ...
        _register_next_step_handler(msg, step_edit_product_price)
...
def step_quick_setup(message):
    ...
    try:
        ...
        _register_next_step_handler(msg, step_quick_setup)
...
def step_get_reseller_uid(message):
    try:
        ...
        _register_next_step_handler(msg, step_get_reseller_uid)
...
def step_add_hack_name(message):
    ...
    _register_next_step_handler(msg, step_add_hack_days)
...
def step_add_hack_days(message):
    ...
    _register_next_step_handler(msg, step_add_hack_price)
...
def step_add_hack_price(message):
    ...
    try:
        ...
        _register_next_step_handler(msg, step_add_hack_resell_price)
    except ValueError:
...
# ==================== MULTI NEW API SERVER MANAGEMENT ====================
def _normalize_api_server_url(value):
    url = str(value or "").strip().strip("`").strip()
    if not url:
        return ""
    # Admin may paste the balance endpoint from curl. Store the server root.
    url = re.sub(r"/api/v1/balance/?$", "", url, flags=re.I)
    url = re.sub(r"/api/v1/?$", "", url, flags=re.I)
    return url.rstrip("/")


def _api_server_row(server_id):
    conn = sqlite3.connect("shop_data.db", timeout=15)
    try:
        return conn.execute(
            """SELECT id, server_name, api_key, server_url, enabled, created_at,
                      COALESCE(last_connected_at,''), COALESCE(account_name,''),
                      COALESCE(account_username,''), COALESCE(account_user_id,'')
               FROM api_servers WHERE id=?""",
            (int(server_id),)
        ).fetchone()
    finally:
        conn.close()
    ...
    try:
        return conn.execute(
            """SELECT id, server_name, api_key, server_url, enabled, created_at,
                      COALESCE(last_connected_at,''), COALESCE(account_name,''),
                      COALESCE(account_username,''), COALESCE(account_user_id,'')
               FROM api_servers ORDER BY id ASC"""
        ).fetchall()
    finally:
        conn.close()


def _api_server_save(server_name, api_key, server_url):
    server_url = _normalize_api_server_url(server_url)
    conn = sqlite3.connect("shop_data.db", timeout=15)
    try:
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO api_servers(server_name,api_key,server_url,enabled,created_at,last_connected_at,account_name,account_username,account_user_id) VALUES(?,?,?,?,?,?,?,?,?)",
            (str(server_name).strip(), str(api_key).strip(), server_url, 0,
             datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), '', '', '', '')
        )
        sid = cur.lastrowid
        conn.commit()
...
def _api_server_set_enabled(server_id, enabled):
    conn = sqlite3.connect("shop_data.db", timeout=15)
    try:
        conn.execute("UPDATE api_servers SET enabled=? WHERE id=?", (1 if enabled else 0, int(server_id)))
        conn.commit()
    finally:
        conn.close()


def _api_server_save_product(server_id, product_name, product_id):
    conn = sqlite3.connect("shop_data.db", timeout=15)
    try:
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        conn.execute(
            """INSERT INTO api_server_products(server_id,product_name,product_id,updated_at)
               VALUES(?,?,?,?)
               ON CONFLICT(server_id,product_name) DO UPDATE SET
               product_id=excluded.product_id, updated_at=excluded.updated_at""",
            (int(server_id), str(product_name), str(product_id).strip(), now)
        )
        # This product uses this New API Server for future automatic delivery.
        conn.execute(
            """INSERT INTO product_server_selection(product_name,server_id,updated_at)
               VALUES(?,?,?)
               ON CONFLICT(product_name) DO UPDATE SET
               server_id=excluded.server_id, updated_at=excluded.updated_at""",
            (str(product_name), int(server_id), now)
        )
        conn.commit()
    finally:
        conn.close()
...
def _api_server_product(server_id, product_name):
    conn = sqlite3.connect("shop_data.db", timeout=15)
    try:
        return conn.execute(
            "SELECT server_id,product_name,product_id,updated_at FROM api_server_products WHERE server_id=? AND product_name=?",
            (int(server_id), str(product_name))
        ).fetchone()
    finally:
        conn.close()


def _api_server_save_plan(server_id, product_name, plan_id, plan_days, valid_id):
    conn = sqlite3.connect("shop_data.db", timeout=15)
    try:
        now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        conn.execute(
            """INSERT INTO api_server_plan_ids(server_id,product_name,plan_id,valid_id,plan_days,updated_at)
               VALUES(?,?,?,?,?,?)
               ON CONFLICT(server_id,product_name,plan_id) DO UPDATE SET
               valid_id=excluded.valid_id, plan_days=excluded.plan_days, updated_at=excluded.updated_at""",
            (int(server_id), str(product_name), int(plan_id), str(valid_id).strip(), str(plan_days), now)
        )
        conn.commit()
    finally:
        conn.close()
...
def _api_server_plan_valid_id(server_id, product_name, plan_id):
    conn = sqlite3.connect("shop_data.db", timeout=15)
    try:
        row = conn.execute(
            "SELECT valid_id FROM api_server_plan_ids WHERE server_id=? AND product_name=? AND plan_id=?",
            (int(server_id), str(product_name), int(plan_id))
        ).fetchone()
...
def _api_server_product_id(server_id, product_name):
    row = _api_server_product(server_id, product_name)
    return str(row[2]).strip() if row and row[2] else ""



def _api_server_set_connection_meta(server_id, account_name='', account_username='', account_user_id='', connected_at=None):
    """Persist the last successful v1 connection and live reseller identity.

    This is additive metadata only. It never changes Product IDs, Valid IDs or
    existing server configuration, and it makes the server list show the same
    connected/date information the reseller dashboard exposes.
    """
    connected_at = connected_at or datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    conn = sqlite3.connect("shop_data.db", timeout=15)
    try:
        conn.execute(
            """UPDATE api_servers
               SET enabled=1, last_connected_at=?, account_name=?, account_username=?, account_user_id=?
               WHERE id=?""",
            (str(connected_at), str(account_name or ''), str(account_username or ''),
             str(account_user_id or ''), int(server_id))
        )
        conn.commit()
    finally:
        conn.close()
...
def _api_server_extract_identity(data):
    """Extract reseller identity from /me without assuming one exact response shape."""
    if not isinstance(data, dict):
        return '', '', ''

    candidates = [data]
    for key in ('data', 'account', 'user', 'reseller', 'profile'):
        value = data.get(key)
    ...
    name = first_value(('name', 'display_name', 'reseller_name', 'full_name', 'label'))
    username = first_value(('username', 'telegram_username', 'handle'))
    user_id = first_value(('user_id', 'telegram_id', 'reseller_id', 'id'))
    return name, username, user_id


def _api_server_parse_json(response):
    try:
        value = response.json()
...
def _api_server_test_connection(server_id):
    row = _api_server_row(server_id)
    if not row:
        return False, "Server not found."

    _, _, api_key, server_url, _, _, last_connected_at, account_name, account_username, account_user_id = row
    server_url = _normalize_api_server_url(server_url)
    api_key = str(api_key or '').strip()
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Accept": "application/json",
        "User-Agent": "RG-Cheat-Shop-Bot/1.0",
    }

    # The new server shown in the supplied integration guide is a REST v1 API:
    # GET /api/v1/balance with Bearer authentication. Do NOT mark a server
    # connected from the old legacy root endpoint unless the v1 route genuinely
    # does not exist; otherwise a 200 from an unrelated legacy handler can make
    # the UI say Connected while purchases still fail.
    detail = ""
    try:
        res = HTTP_SESSION.get(f"{server_url}/api/v1/balance", headers=headers, timeout=10)
        raw = res.text[:500]
        if res.status_code == 200:
            identity_name, identity_username, identity_user_id = account_name, account_username, account_user_id

            # /me is explicitly documented by the supplied server as account + balance.
            # It is used only to enrich the connection record; /balance already proves auth.
            try:
                me = HTTP_SESSION.get(f"{server_url}/api/v1/me", headers=headers, timeout=8)
                if me.status_code == 200:
                    parsed_me = _api_server_parse_json(me)
                    n, u, uid = _api_server_extract_identity(parsed_me)
                    identity_name = n or identity_name
                    identity_username = u or identity_username
                    identity_user_id = uid or identity_user_id
            except Exception:
                pass

            connected_at = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            _api_server_set_connection_meta(
                server_id,
                identity_name,
                identity_username,
                identity_user_id,
        ...
        detail = f"HTTP {res.status_code}: {raw}"
        # 401/403 means the supplied Bearer key is wrong/disabled. Do not fall
        # through to the legacy contract and falsely report a successful server.
        if res.status_code not in (404, 405):
            _api_server_set_enabled(server_id, False)
            return False, detail[:300]
    except Exception as exc:
        detail = str(exc)[:300]

    # Compatibility fallback for the older reseller-style API contract. This is
    # intentionally limited to a missing v1 route so the new REST API cannot be
    # accidentally marked connected by a legacy endpoint.
    try:
        res = HTTP_SESSION.post(
            server_url,
            data={"api_key": api_key, "action": "balance"},
            headers={"Content-Type": "application/x-www-form-urlencoded", "x-master-key": RESELLER_MASTER_KEY},
            timeout=10,
        )
        if res.status_code == 200:
            _api_server_set_enabled(server_id, True)
...
def _api_servers_markup():
    markup = types.InlineKeyboardMarkup()
    servers = _api_servers()
    for sid, name, api_key, url, enabled, created_at, last_connected_at, account_name, account_username, account_user_id in servers:
        status = "🟢" if enabled else "⚪"
        # Keep the button compact; detailed identity/date is shown after tapping the server.
        label = f"{status} {name}"
        markup.add(types.InlineKeyboardButton(label[:64], callback_data=f"api_srv_{sid}"))
    markup.add(types.InlineKeyboardButton("➕ New API Server", callback_data="api_srv_add"))
    markup.add(types.InlineKeyboardButton("🔙 Back", callback_data="admin_api_system"))
    return markup


def _api_server_hacks_markup(server_id):
    markup = types.InlineKeyboardMarkup()
    for prod in get_unique_products():
        pid = _api_server_product_id(server_id, prod)
        status = "🟢" if pid else "⚪"
        markup.add(types.InlineKeyboardButton(
            f"{status} {prod}",
            callback_data=f"api_hack_{int(server_id)}_{_ui_hash(prod)}"
        ))
    markup.add(types.InlineKeyboardButton("🔙 Server", callback_data=f"api_srv_{int(server_id)}"))
    return markup


def _api_server_plans_markup(server_id, product_name):
    markup = types.InlineKeyboardMarkup()
    pid = _api_server_product_id(server_id, product_name) or "—"
    for plan in get_product_plans(product_name):
        plan_id = int(plan[0])
        days = str(plan[2])
        valid_id = _api_server_plan_valid_id(server_id, product_name, plan_id)
        status = "🟢" if valid_id else "⚪"
        markup.add(types.InlineKeyboardButton(
            f"{status} {days} | PID:{pid} | VID:{valid_id or '—'}"[:60],
            callback_data=f"api_plan_{int(server_id)}_{_ui_hash(product_name)}_{plan_id}"
        ))
    markup.add(types.InlineKeyboardButton(
        "🆔 Set / Change Product ID",
        callback_data=f"api_pid_{int(server_id)}_{_ui_hash(product_name)}"
    ))
    markup.add(types.InlineKeyboardButton(
        "🔙 Hack List", callback_data=f"api_hacks_{int(server_id)}"
    ))
    return markup


def _api_server_hack_by_hash(value):
    for product_name in get_unique_products():
        if _ui_hash(product_name) == str(value):
            return product_name
    return None


def _show_api_server_list(chat_id, message_id=None):
    text = (
        "🆕 <b>NEW API SERVER</b>\n\n"
        "Add multiple New API Servers and configure each hack separately.\n"
        "🟢 = connected • ⚪ = not checked / disconnected\n\n"
        "Each hack keeps its own Product ID and every plan keeps its own Valid ID."
    )
    markup = _api_servers_markup()
    if message_id:
        try:
...
def step_save_api_server_key(message):
    uid = message.from_user.id
    if uid not in ADMIN_IDS:
        return
    key = (message.text or "").strip()
    if not key:
        m = bot.send_message(message.chat.id, "❌ API Key cannot be empty. Enter the API Key again.")
        _register_next_step_handler(m, step_save_api_server_key)
        return
    admin_temp_data.setdefault(uid, {})["api_server_key"] = key
    m = bot.send_message(
        message.chat.id,
        "🌐 <b>Enter New API Server Link:</b>\n\n"
        "Example: <code>https://example.com</code>\n"
        "You can also paste the <code>/api/v1/balance</code> URL; it will be normalized automatically.",
        parse_mode="HTML"
    )
    _register_next_step_handler(m, step_save_api_server_url)


def step_save_api_server_url(message):
    uid = message.from_user.id
    if uid not in ADMIN_IDS:
        return
    data = admin_temp_data.get(uid, {})
    url = _normalize_api_server_url(message.text or "")
    if not data.get("api_server_key") or not re.match(r"^https?://", url, re.I):
        m = bot.send_message(message.chat.id, "❌ Invalid Server Link. Enter a valid http:// or https:// URL.")
        _register_next_step_handler(m, step_save_api_server_url)
        return
    count = len(_api_servers()) + 1
    server_name = f"New Server {count}"
    sid = _api_server_save(server_name, data["api_server_key"], url)
    admin_temp_data.pop(uid, None)
    ok, detail = _api_server_test_connection(sid)
    status = "🟢 <b>Connected Successfully!</b>" if ok else "🔴 <b>Not Connect</b>"
    bot.send_message(
        message.chat.id,
        f"✅ <b>API Server Added Successfully!</b>\n\n"
        f"🌐 <b>Server:</b> {html.escape(server_name)}\n"
        f"🔗 <b>Link:</b> <code>{html.escape(url)}</code>\n"
        f"{status}\n\n"
        f"Tap <b>Configure Hacks</b> to map Product IDs and Valid IDs.",
        parse_mode="HTML",
        reply_markup=types.InlineKeyboardMarkup()
    )
    # Rebuild keyboard after the success message so the server is immediately usable.
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("⚙️ Configure Hacks", callback_data=f"api_hacks_{sid}"))
    markup.add(types.InlineKeyboardButton("🔍 Check Connection", callback_data=f"api_check_{sid}"))
    markup.add(types.InlineKeyboardButton("🔙 API Servers", callback_data="new_server_api"))
    bot.send_message(message.chat.id, "🧩 <b>Server Controls</b>", parse_mode="HTML", reply_markup=markup)


def _resolve_multi_api_route(product_name, plan_id=None, local_pid_id=None, duration=None):
    """Resolve a configured per-product New API Server before legacy routes."""
    if product_name:
        conn = sqlite3.connect("shop_data.db", timeout=15)
        try:
            row = conn.execute(
                """SELECT s.id,s.api_key,s.server_url,s.enabled,p.product_id
                   FROM product_server_selection sel
                   JOIN api_servers s ON s.id=sel.server_id
                   JOIN api_server_products p
                     ON p.server_id=s.id AND p.product_name=sel.product_name
                   WHERE sel.product_name=? LIMIT 1""",
                (str(product_name),)
            ).fetchone()
        finally:
            conn.close()
        if row:
            sid, api_key, server_url, enabled, product_id = row
            valid_id = _api_server_plan_valid_id(sid, product_name, plan_id) if plan_id is not None else ""
            if product_id and (plan_id is None or valid_id):
                return {
                    "url": _normalize_api_server_url(server_url),
                    "api_key": api_key,
                    "product_id": product_id,
                    "valid_id": valid_id,
                    "server_type": "new_multi",
                    "server_id": sid,
                }
    return None


# Replace route resolution at runtime with a wrapper that preserves the old
# New Server and Old Server fallbacks exactly as before.
_legacy_resolve_api_route = _resolve_api_route
def _resolve_api_route(product_name, plan_id=None, local_pid_id=None, duration=None):
    multi = _resolve_multi_api_route(product_name, plan_id, local_pid_id, duration)
    if multi:
        return multi
    return _legacy_resolve_api_route(product_name, plan_id, local_pid_id, duration)



def _is_command_input(text):
    """Return True when a Telegram command was sent to a numeric/API-ID prompt.

    Next-step handlers receive messages before normal command handlers, so /start
    could previously be accidentally saved as a Product ID/Valid ID.
    """
    value = str(text or "").strip()
    return value.startswith("/")

def _cancel_pending_id_step(uid):
    try:
        bot.clear_step_handler_by_chat_id(uid)
    except Exception:
        pass

def step_save_multi_api_product_id(message):
    uid = message.from_user.id
    data = admin_temp_data.get(uid, {})
    sid = data.get("api_server_id")
    product_name = data.get("api_server_product")
    pid = (message.text or "").strip()
    if not sid or not product_name:
        bot.send_message(message.chat.id, "❌ Session expired.")
        return
    if _is_command_input(pid):
        _cancel_pending_id_step(uid)
        admin_temp_data.pop(uid, None)
        if pid.lower().split()[0] == "/start":
            send_welcome(message)
        else:
            bot.send_message(message.chat.id, "ℹ️ ID entry cancelled. Please open Configure Hacks again.")
        return
    if not pid:
        m = bot.send_message(message.chat.id, "❌ Product ID cannot be empty. Enter it again.")
        _register_next_step_handler(m, step_save_multi_api_product_id)
        return
    if not re.fullmatch(r"\d+", pid):
        m = bot.send_message(message.chat.id, "❌ Invalid Product ID. Enter the numeric Product ID only (example: <code>82</code>).", parse_mode="HTML")
        _register_next_step_handler(m, step_save_multi_api_product_id)
        return
    _api_server_save_product(sid, product_name, pid)
    admin_temp_data.pop(uid, None)
    bot.send_message(
        message.chat.id,
        f"✅ <b>Product ID Saved Successfully!</b>\n\n"
        f"🎮 <b>Hack:</b> {html.escape(product_name)}\n"
        f"🆔 <b>Product ID:</b> <code>{html.escape(pid)}</code>\n\n"
        "Now set each plan's Valid ID.",
        parse_mode="HTML",
        reply_markup=_api_server_plans_markup(sid, product_name)
    )


def step_save_multi_api_valid_id(message):
    uid = message.from_user.id
    data = admin_temp_data.get(uid, {})
    sid = data.get("api_server_id")
    product_name = data.get("api_server_product")
    plan_id = data.get("api_server_plan_id")
    plan_days = data.get("api_server_plan_days", "")
    valid_id = (message.text or "").strip()
    if not sid or not product_name or plan_id is None:
        bot.send_message(message.chat.id, "❌ Session expired.")
        return
    if _is_command_input(valid_id):
        _cancel_pending_id_step(uid)
        admin_temp_data.pop(uid, None)
        if valid_id.lower().split()[0] == "/start":
            send_welcome(message)
        else:
            bot.send_message(message.chat.id, "ℹ️ Valid ID entry cancelled. Please open Configure Hacks again.")
        return
    if not valid_id:
        m = bot.send_message(message.chat.id, "❌ Valid ID cannot be empty. Enter it again.")
        _register_next_step_handler(m, step_save_multi_api_valid_id)
        return
    if not re.fullmatch(r"\d+", valid_id):
        m = bot.send_message(message.chat.id, "❌ Invalid Valid ID. Enter the numeric Valid ID only (example: <code>233</code>).", parse_mode="HTML")
        _register_next_step_handler(m, step_save_multi_api_valid_id)
        return
    if not _api_server_product_id(sid, product_name):
        bot.send_message(message.chat.id, "❌ Set the Product ID first.", parse_mode="HTML")
        admin_temp_data.pop(uid, None)
        return
    _api_server_save_plan(sid, product_name, int(plan_id), str(plan_days), valid_id)
    admin_temp_data.pop(uid, None)
    bot.send_message(
        message.chat.id,
        f"✅ <b>Valid ID Saved Successfully!</b>\n\n"
        f"🎮 <b>Hack:</b> {html.escape(product_name)}\n"
        f"📅 <b>Plan:</b> {html.escape(str(plan_days))}\n"
        f"🔢 <b>Valid ID:</b> <code>{html.escape(valid_id)}</code>",
        parse_mode="HTML",
        reply_markup=_api_server_plans_markup(sid, product_name)
    )


def step_save_new_server_product_id(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{}) ; product_name=d.get('new_server_product'); pid=(message.text or '').strip()
    if not product_name: bot.send_message(message.chat.id,"❌ Session expired."); return
    if _is_command_input(pid):
        _cancel_pending_id_step(uid); admin_temp_data.pop(uid,None)
        if pid.lower().split()[0] == "/start": send_welcome(message)
        else: bot.send_message(message.chat.id,"ℹ️ ID entry cancelled. Please open Configure Hacks again.")
        return
    if not pid:
        m=bot.send_message(message.chat.id,"❌ Product ID cannot be empty. Enter the exact Product ID:"); _register_next_step_handler(m,step_save_new_server_product_id); return
    if not re.fullmatch(r"\d+", pid):
        m=bot.send_message(message.chat.id,"❌ Invalid Product ID. Enter the numeric Product ID only (example: <code>82</code>).",parse_mode="HTML"); _register_next_step_handler(m,step_save_new_server_product_id); return
    _save_new_server_config(product_name,NEW_SERVER_API_KEY,NEW_SERVER_API_URL,pid); admin_temp_data.pop(uid,None)
    bot.send_message(message.chat.id,f"✅ <b>New Server Product ID saved.</b>\n\n🎮 <b>Hack:</b> {html.escape(product_name)}\n🆔 <b>Product ID:</b> <code>{html.escape(pid)}</code>",parse_mode="HTML",reply_markup=_new_server_hacks_markup())

def step_save_new_server_valid_id(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{}) ; product_name=d.get('new_server_product'); plan_id=d.get('new_server_plan_id'); plan_days=d.get('new_server_plan_days',''); valid_id=(message.text or '').strip()
    if not product_name or plan_id is None: bot.send_message(message.chat.id,"❌ Session expired."); return
    if _is_command_input(valid_id):
        _cancel_pending_id_step(uid); admin_temp_data.pop(uid,None)
        if valid_id.lower().split()[0] == "/start": send_welcome(message)
        else: bot.send_message(message.chat.id,"ℹ️ Valid ID entry cancelled. Please open Configure Hacks again.")
        return
    if not valid_id:
        m=bot.send_message(message.chat.id,"❌ Valid ID cannot be empty. Enter the exact Valid ID:"); _register_next_step_handler(m,step_save_new_server_valid_id); return
    if not re.fullmatch(r"\d+", valid_id):
        m=bot.send_message(message.chat.id,"❌ Invalid Valid ID. Enter the numeric Valid ID only (example: <code>233</code>).",parse_mode="HTML"); _register_next_step_handler(m,step_save_new_server_valid_id); return
    cfg=_new_server_config(product_name)
    if not cfg or not cfg[3]: bot.send_message(message.chat.id,"❌ Set the New Server Product ID first.",parse_mode="HTML"); admin_temp_data.pop(uid,None); return
    _save_new_server_plan_valid_id(product_name,int(plan_id),str(plan_days),valid_id); admin_temp_data.pop(uid,None)
    bot.send_message(message.chat.id,f"✅ <b>Valid ID saved.</b>\n\n🎮 <b>Hack:</b> {html.escape(product_name)}\n📅 <b>Plan:</b> {html.escape(str(plan_days))}\n🆔 <b>Product ID:</b> <code>{html.escape(str(cfg[3]))}</code>\n🔢 <b>Valid ID:</b> <code>{html.escape(valid_id)}</code>",parse_mode="HTML",reply_markup=_new_server_plans_markup(product_name))

def _server_manager_hacks_markup():
    markup=types.InlineKeyboardMarkup()
    for prod in get_unique_products(): markup.add(types.InlineKeyboardButton(f"🎮 {prod}",callback_data=f"hack_server_select_{prod}"))
    return markup

def step_save_hack_server_api_key(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{})
    if not d.get('hack_server_product'): return bot.send_message(message.chat.id,"❌ Session expired.")
    d['api_key']=(message.text or '').strip()
    if not d['api_key']:
        m=bot.send_message(message.chat.id,"❌ API Key cannot be empty."); _register_next_step_handler(m,step_save_hack_server_api_key); return
    m=bot.send_message(message.chat.id,"🌐 <b>Enter Server URL:</b>",parse_mode="HTML"); _register_next_step_handler(m,step_save_hack_server_url)

def step_save_hack_server_url(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{}) ; url=(message.text or '').strip()
    if not d.get('hack_server_product') or not url.startswith(('http://','https://')):
        m=bot.send_message(message.chat.id,"❌ Enter a valid Server URL:"); _register_next_step_handler(m,step_save_hack_server_url); return
    d['server_url']=url; m=bot.send_message(message.chat.id,"🆔 <b>Enter Server Product ID:</b>",parse_mode="HTML"); _register_next_step_handler(m,step_save_hack_server_product_id)

def step_save_hack_server_product_id(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{}) ; pid=(message.text or '').strip()
    if not d.get('hack_server_product') or not pid:
        m=bot.send_message(message.chat.id,"❌ Product ID cannot be empty."); _register_next_step_handler(m,step_save_hack_server_product_id); return
    d['remote_product_id']=pid; plans=get_product_plans(d['hack_server_product']); d['hack_server_plan_queue']=[{'plan_id':int(x[0]),'days':str(x[2])} for x in plans]; d['hack_server_plan_index']=0
    _save_hack_server_config(d['hack_server_product'],d['api_key'],d['server_url'],pid)
    if not d['hack_server_plan_queue']:
        bot.send_message(message.chat.id,"✅ <b>Server saved!</b>",parse_mode="HTML"); admin_temp_data.pop(uid,None); return
    _ask_next_hack_valid_id(message.chat.id,uid)

def _ask_next_hack_valid_id(chat_id,uid):
    d=admin_temp_data.get(uid,{}); q=d.get('hack_server_plan_queue',[]); i=int(d.get('hack_server_plan_index',0))
    if i>=len(q):
        bot.send_message(chat_id,f"✅ <b>Hack API & Server configuration saved!</b>\n\n🎮 <b>Hack:</b> {html.escape(d.get('hack_server_product',''))}\n🌐 <b>Server:</b> <code>{html.escape(d.get('server_url',''))}</code>\n🆔 <b>Product ID:</b> <code>{html.escape(d.get('remote_product_id',''))}</code>\n\nThis hack now uses the new server; other hacks remain unchanged.",parse_mode="HTML"); admin_temp_data.pop(uid,None); return
    plan=q[i]; m=bot.send_message(chat_id,f"🆔 <b>Enter Valid ID for {html.escape(plan['days'])}:</b>\n\nPlan {i+1} of {len(q)}",parse_mode="HTML"); _register_next_step_handler(m,step_save_hack_valid_id)

def step_save_hack_valid_id(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{}); q=d.get('hack_server_plan_queue',[]); i=int(d.get('hack_server_plan_index',0)); valid=(message.text or '').strip()
    if not q or i>=len(q): admin_temp_data.pop(uid,None); return
    if not valid:
        m=bot.send_message(message.chat.id,"❌ Valid ID cannot be empty."); _register_next_step_handler(m,step_save_hack_valid_id); return
    plan=q[i]; _save_hack_server_plan_valid_id(d['hack_server_product'],plan['plan_id'],plan['days'],valid); d['hack_server_plan_index']=i+1; _ask_next_hack_valid_id(message.chat.id,uid)
...
def step_get_server_token(message):
    ...
    _register_next_step_handler(msg, step_get_server_url)
...
def step_get_remote_pid(message):
    ...
    _register_next_step_handler(msg, step_get_remote_duration)
...
def step_get_new_bot_token(message):
    ...
    if ":" in new_token and len(new_token) > 20:
        ...
        _register_next_step_handler(msg, step_get_new_bot_token)
...
def step_get_user_discount_id(message):
    ...
    try:
        ...
        _register_next_step_handler(msg, step_get_user_discount_percent)
    except ValueError:
        msg = bot.send_message(message.chat.id, "❌ Invalid User ID! Please enter numeric ID:")
        _register_next_step_handler(msg, step_get_user_discount_id)
...
def step_get_global_discount_percent(message):
    try:
        ...
        msg = bot.send_message(message.chat.id, "❌ Enter valid percentage number:")
        _register_next_step_handler(msg, step_get_global_discount_percent)
...
def step_get_prod_discount_percent(message):
    ...
    try:
        ...
        msg = bot.send_message(message.chat.id, "❌ Enter valid percentage number:")
        _register_next_step_handler(msg, step_get_prod_discount_percent)
...
def step_get_bal_user_id(message):
    ...
    try:
        ...
        _register_next_step_handler(msg, step_get_bal_user_id)
...
def step_get_bal_amount(message):
    ...
    try:
        ...
        _register_next_step_handler(msg, step_get_bal_amount)
...
def step_get_guide_url(message):
    ...
    if new_url.startswith("http://") or new_url.startswith("https://"):
        ...
        _register_next_step_handler(msg, step_get_guide_url)
...
def step_get_custom_topup(message):
    try:
        ...
        if 1 <= amount <= 10000:
            ...
            _register_next_step_handler(msg, step_get_custom_topup)
    except ValueError:
...
def step_save_hack_price_ui_title(message):
    ...
    if not raw:
        ...
        return
    # Validate that the message contains the dynamic labels/placeholders needed
    # by the live renderer. Premium <tg-emoji> wrappers are ignored for validation.
    plain = re.sub(r'<tg-emoji\b[^>]*>.*?</tg-emoji>', '', raw, flags=re.I | re.S)
    plain = re.sub(r'<[^>]+>', '', plain).lower()
    has_validity = 'validity' in plain or '{validity}' in plain or '{days}' in plain
    has_stock = 'stock' in plain or '{stock}' in plain or '{stock_status}' in plain
    has_price = 'price' in plain or '{price}' in plain or '{price_text}' in plain
    if not (has_validity and has_stock and has_price):
        bot.send_message(
            message.chat.id,
            "❌ Your design must contain <b>Validity</b>, <b>Stock</b> and <b>Price</b> (or their placeholders) so the bot can inject live values automatically.",
            parse_mode="HTML"
        )
        return
...
def step_save_welcome_message(message):
    if message.from_user.id not in ADMIN_IDS: return
    text=_message_html_text(message)
    if not text.strip():
        bot.send_message(message.chat.id,"❌ Welcome Message cannot be empty.")
        return
    set_welcome_message_template(text)
    bot.send_message(message.chat.id,"✅ <b>Welcome Message Updated Successfully!</b>",parse_mode="HTML")
...
def show_product_plans(chat_id, product_name, message_id=None):
    ...
    for plan in plans:
        ...
        live_rows.append({
            ...
            "discount_percent": (0.0 if is_reseller == 1 else float(effective_discount)),
        })
...
def process_wallet_checkout(chat_id, user_id, product_id, device_id=None):
    ...
    if deduct_user_balance(user_id, final_price):
        ...
        if not key_val:
            key_val = fetch_key_from_api(pid_id, remote_dur, android_id=device_id, product_name=prod_name, plan_id=product[0], purchase_ref=order_id)
        if not key_val:
            # Never charge a wallet and then save a fake/placeholder key when the
            # configured New REST API did not actually deliver one.
            refund_user_balance(user_id, final_price)
            bot.send_message(
                chat_id,
                "❌ <b>Key delivery failed.</b>\n\n"
                "The API server was connected, but it did not return a key for this Product ID / Valid ID.\n"
                "Your wallet balance has been refunded. Please check the Product ID and Valid ID mapping.",
                parse_mode="HTML"
            )
            return

        record_order(order_id, user_id, prod_name, prod_days, final_price, key_val)
        notify_admins_purchase(user_id, first_name, order_id, prod_name, prod_days, final_price, device_id, "wallet", key_val)
...
# ==================== NEW SIMPLIFIED ADMIN SYSTEMS ====================
def _admin_back_markup(callback='admin_main'):
    m=types.InlineKeyboardMarkup(); m.add(types.InlineKeyboardButton('🔙 Back',callback_data=callback)); return m

def _all_settings_hack_markup():
    m=types.InlineKeyboardMarkup()
    for name in get_unique_products():
        m.add(types.InlineKeyboardButton(f'🎮 {name}', callback_data=f'allset_hack_{_ui_hash(name)}'))
    m.add(types.InlineKeyboardButton('➕', callback_data='allset_add_hack'))
    m.add(types.InlineKeyboardButton('🔙 Main Admin Panel', callback_data='admin_main'))
    return m

def _all_settings_show(chat_id, message_id=None):
    text='⚙️ <b>ALL SETTINGS</b>\n\nManage hacks and plans from one simple panel.\n\nTap a hack to edit its plans, or tap ➕ to add a new hack.'
    markup=_all_settings_hack_markup()
    if message_id:
        try: bot.edit_message_text(text,chat_id,message_id,parse_mode='HTML',reply_markup=markup); return
        except Exception: pass
    bot.send_message(chat_id,text,parse_mode='HTML',reply_markup=markup)

def _all_settings_plan_markup(product_name):
    m=types.InlineKeyboardMarkup()
    for plan in get_product_plans(product_name):
        m.add(types.InlineKeyboardButton(f'📅 {plan[2]}',callback_data=f'allset_plan_{int(plan[0])}'))
    m.add(types.InlineKeyboardButton('➕',callback_data=f'allset_add_plan_{_ui_hash(product_name)}'))
    m.add(types.InlineKeyboardButton('✏️ Edit',callback_data=f'allset_edit_hack_{_ui_hash(product_name)}'))
    m.add(types.InlineKeyboardButton('🔙 Back',callback_data='all_settings_open'))
    return m

def _all_settings_hack_by_hash(value):
    for name in get_unique_products():
        if _ui_hash(name)==str(value): return name
    return None

def _all_settings_plan_prompt(chat_id, product_name):
    text=f'🎮 <b>{html.escape(product_name)}</b>\n\nSelect a plan to edit, or tap ➕ to add another plan.'
    bot.send_message(chat_id,text,parse_mode='HTML',reply_markup=_all_settings_plan_markup(product_name))

def _start_all_settings_add_hack(message):
    uid=message.from_user.id
    if uid not in ADMIN_IDS: return
    admin_temp_data[uid]={'flow':'allset_add_hack'}
    m=bot.send_message(message.chat.id,'🎮 <b>Enter Hack Name:</b>\n\nSend /cancel to stop.',parse_mode='HTML')
    _register_next_step_handler(m,_all_settings_add_hack_name)

def _all_settings_add_hack_name(message):
    uid=message.from_user.id; name=(message.text or '').strip()
    if not name: m=bot.send_message(message.chat.id,'❌ Hack name cannot be empty. Enter again:'); _register_next_step_handler(m,_all_settings_add_hack_name); return
    admin_temp_data[uid]={'flow':'allset_add_plan','name':name}
    m=bot.send_message(message.chat.id,'📅 <b>Enter Plan / Days:</b>\nExample: <code>1 Day</code> or <code>3 Days</code>',parse_mode='HTML')
    _register_next_step_handler(m,_all_settings_add_plan_days)

def _all_settings_add_plan_days(message):
    uid=message.from_user.id; days=(message.text or '').strip(); d=admin_temp_data.get(uid,{})
    if not days: m=bot.send_message(message.chat.id,'❌ Plan/Days cannot be empty. Enter again:'); _register_next_step_handler(m,_all_settings_add_plan_days); return
    d['days']=days
    m=bot.send_message(message.chat.id,'💰 <b>Enter User Price:</b>\nExample: <code>100</code>'); _register_next_step_handler(m,_all_settings_add_user_price)

def _all_settings_add_user_price(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{})
    try: d['price']=float((message.text or '').strip())
    except Exception:
        m=bot.send_message(message.chat.id,'❌ Invalid price. Enter a number:'); _register_next_step_handler(m,_all_settings_add_user_price); return
    m=bot.send_message(message.chat.id,'💎 <b>Enter Reseller Price:</b>\nExample: <code>80</code>'); _register_next_step_handler(m,_all_settings_add_reseller_price)

def _all_settings_add_reseller_price(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{})
    try: r=float((message.text or '').strip())
    except Exception:
        m=bot.send_message(message.chat.id,'❌ Invalid reseller price. Enter a number:'); _register_next_step_handler(m,_all_settings_add_reseller_price); return
    d['resell_price']=r; name=d['name']; days=d['days']; add_product_db(name,'0',days,d['price'],r,is_manual=0,remote_duration=days)
    admin_temp_data.pop(uid,None)
    bot.send_message(message.chat.id,f'✅ <b>Saved Successfully!</b>\n\n🎮 <b>Hack:</b> {html.escape(name)}\n📅 <b>Plan:</b> {html.escape(days)}\n💰 <b>User:</b> ₹{d["price"]:.2f}\n💎 <b>Reseller:</b> ₹{r:.2f}',parse_mode='HTML',reply_markup=_all_settings_plan_markup(name))

def _start_all_settings_add_plan(message, product_name):
    uid=message.from_user.id; admin_temp_data[uid]={'flow':'allset_existing_plan','name':product_name}
    m=bot.send_message(message.chat.id,f'📅 <b>New plan for {html.escape(product_name)}</b>\n\nEnter Plan / Days:',parse_mode='HTML'); _register_next_step_handler(m,_all_settings_existing_plan_days)

def _all_settings_existing_plan_days(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{})
    days=(message.text or '').strip()
    if not days: m=bot.send_message(message.chat.id,'❌ Enter Plan / Days again:'); _register_next_step_handler(m,_all_settings_existing_plan_days); return
    d['days']=days; m=bot.send_message(message.chat.id,'💰 <b>Enter User Price:</b>'); _register_next_step_handler(m,_all_settings_existing_plan_user_price)

def _all_settings_existing_plan_user_price(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{})
    try: d['price']=float((message.text or '').strip())
    except Exception: m=bot.send_message(message.chat.id,'❌ Invalid price. Enter again:'); _register_next_step_handler(m,_all_settings_existing_plan_user_price); return
    m=bot.send_message(message.chat.id,'💎 <b>Enter Reseller Price:</b>'); _register_next_step_handler(m,_all_settings_existing_plan_reseller_price)

def _all_settings_existing_plan_reseller_price(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{})
    try: r=float((message.text or '').strip())
    except Exception: m=bot.send_message(message.chat.id,'❌ Invalid reseller price. Enter again:'); _register_next_step_handler(m,_all_settings_existing_plan_reseller_price); return
    add_product_db(d['name'],'0',d['days'],d['price'],r,is_manual=0,remote_duration=d['days']); admin_temp_data.pop(uid,None)
    bot.send_message(message.chat.id,'✅ <b>Plan Added Successfully!</b>',parse_mode='HTML',reply_markup=_all_settings_plan_markup(d['name']))

def _all_settings_plan_edit_menu(chat_id, plan_id):
    plan=get_product_by_id(plan_id)
    if not plan: return
    name=plan[1]
    text=(f'✏️ <b>EDIT PLAN</b>\n\n🎮 <b>Hack:</b> {html.escape(name)}\n📅 <b>Days:</b> {html.escape(str(plan[3]))}\n💰 <b>User Price:</b> ₹{plan[4]:.2f}\n💎 <b>Reseller Price:</b> ₹{plan[5]:.2f}')
    m=types.InlineKeyboardMarkup()
    m.add(types.InlineKeyboardButton('📅 Edit Days',callback_data=f'allset_edit_days_{plan_id}'))
    m.add(types.InlineKeyboardButton('💰 Edit User Price',callback_data=f'allset_edit_user_{plan_id}'))
    m.add(types.InlineKeyboardButton('💎 Edit Reseller Price',callback_data=f'allset_edit_resell_{plan_id}'))
    m.add(types.InlineKeyboardButton('🔙 Back',callback_data=f'allset_hack_{_ui_hash(name)}'))
    bot.send_message(chat_id,text,parse_mode='HTML',reply_markup=m)

def _all_settings_edit_hack_menu(chat_id, product_name):
    m=types.InlineKeyboardMarkup(); m.add(types.InlineKeyboardButton('✏️ Edit Hack Name',callback_data=f'allset_rename_{_ui_hash(product_name)}'))
    m.add(types.InlineKeyboardButton('📋 Edit Plans',callback_data=f'allset_hack_{_ui_hash(product_name)}'))
    m.add(types.InlineKeyboardButton('🔙 Back',callback_data='all_settings_open'))
    bot.send_message(chat_id,f'✏️ <b>EDIT HACK</b>\n\n🎮 {html.escape(product_name)}',parse_mode='HTML',reply_markup=m)

def _save_all_settings_rename(message):
    uid=message.from_user.id; d=admin_temp_data.get(uid,{})
    old=d.get('old_name'); new=(message.text or '').strip()
    ok,reason=rename_product_db(old,new)
    admin_temp_data.pop(uid,None)
    if not ok:
        bot.send_message(message.chat.id,'❌ Could not rename. A hack with that name may already exist.')
    else:
        bot.send_message(message.chat.id,'✅ <b>Hack name updated.</b>',parse_mode='HTML',reply_markup=_all_settings_hack_markup())

def _save_all_settings_plan_field(message, field):
    uid=message.from_user.id
    d=admin_temp_data.get(uid,{})
    pid=d.get('plan_id')
    plan=get_product_by_id(pid)
    if not plan:
        admin_temp_data.pop(uid,None)
    ...
    try:
        ...
        if field=='days':
            ...
            conn.execute('UPDATE products SET days=?, remote_duration=? WHERE id=?',(raw,raw,pid))
        else:
            value=float(raw)
            col='price' if field=='user' else 'resell_price'
            conn.execute(f'UPDATE products SET {col}=? WHERE id=?',(value,pid))
        conn.commit()
        conn.close()
    except Exception:
        try: conn.close()
        except Exception: pass
        m=bot.send_message(message.chat.id,'❌ Invalid value. Enter again:')
        _register_next_step_handler(m,lambda mm:_save_all_settings_plan_field(mm,field))
        return
    admin_temp_data.pop(uid,None)
    bot.send_message(message.chat.id,'✅ <b>Updated Successfully!</b>',parse_mode='HTML',reply_markup=_all_settings_plan_edit_back_markup(pid))

def _all_settings_plan_edit_back_markup(pid):
    m=types.InlineKeyboardMarkup(); m.add(types.InlineKeyboardButton('✏️ Continue Editing',callback_data=f'allset_plan_{pid}'));
    plan=get_product_by_id(pid);
    if plan: m.add(types.InlineKeyboardButton('🔙 Back',callback_data=f'allset_hack_{_ui_hash(plan[1])}'))
    return m

# ---- User System ----
def _user_system_markup():
    m=types.InlineKeyboardMarkup()
    for u in get_all_users(): m.add(types.InlineKeyboardButton(f'👤 {u[1]} · {u[0]}',callback_data=f'usys_user_{u[0]}'))
    m.add(types.InlineKeyboardButton('🔙 Main Admin Panel',callback_data='admin_main')); return m

def _user_system_show(chat_id, message_id=None):
    text='👤 <b>USER SYSTEM</b>\n\nSelect a verified user to manage their account.'
    m=_user_system_markup()
    if message_id:
        try: bot.edit_message_text(text,chat_id,message_id,parse_mode='HTML',reply_markup=m); return
        except Exception: pass
    bot.send_message(chat_id,text,parse_mode='HTML',reply_markup=m)

def _user_detail_markup(uid):
    u=get_user(uid); m=types.InlineKeyboardMarkup()
    if not u: return m
    banned=bool(u[16]) if len(u)>16 else is_user_banned(uid)
    reseller=bool(u[11])
    m.add(types.InlineKeyboardButton('🚫 Unban User' if banned else '🚫 Ban User',callback_data=f'usys_ban_{uid}_{0 if banned else 1}'))
    m.add(types.InlineKeyboardButton('🏷️ User Discount',callback_data=f'usys_discount_{uid}'))
    m.add(types.InlineKeyboardButton('👑 Remove Reseller' if reseller else '👑 Add Reseller',callback_data=f'usys_reseller_{uid}_{0 if reseller else 1}'))
    m.add(types.InlineKeyboardButton('🔗 Create / Refresh Referral Code',callback_data=f'usys_refcode_{uid}'))
    m.add(types.InlineKeyboardButton('📊 User Tracking',callback_data=f'usys_track_{uid}'))
    m.add(types.InlineKeyboardButton('🔙 User List',callback_data='user_system_open'))
    return m

def _user_detail_show(chat_id, uid):
    u=get_user(uid)
    if not u: bot.send_message(chat_id,'❌ User not found.'); return
    code=get_user_referral_code(uid) or 'Not created'
    banned='BANNED' if (len(u)>16 and u[16]) else 'ACTIVE'
    text=(f'👤 <b>USER MANAGEMENT</b>\n────────────────────────\n'
          f'👤 <b>Name:</b> {html.escape(str(u[1]))}\n🆔 <b>Telegram ID:</b> <code>{u[0]}</code>\n'
          f'💰 <b>Balance:</b> ₹{u[4]:.2f}\n👑 <b>Reseller:</b> {"YES" if u[11] else "NO"}\n'
          f'🏷️ <b>Discount:</b> {u[12]:.0f}%\n🚦 <b>Status:</b> {banned}\n🔗 <b>Referral Code:</b> <code>{code}</code>')
    bot.send_message(chat_id,text,parse_mode='HTML',reply_markup=_user_detail_markup(uid))

def _user_tracking_show(chat_id, uid):
    u=get_user(uid); orders=get_user_orders(uid)
    if not u: return
    text=(f'📊 <b>USER TRACKING — A TO Z</b>\n────────────────────────\n'
          f'👤 <b>Name:</b> {html.escape(str(u[1]))}\n🆔 <b>Telegram ID:</b> <code>{u[0]}</code>\n'
          f'📞 <b>Phone:</b> {html.escape(str(u[2] or "N/A"))}\n💰 <b>Balance:</b> ₹{u[4]:.2f}\n'
          f'📦 <b>Products/Hacks Bought:</b> {u[6]}\n💎 <b>Lifetime Spent:</b> ₹{u[7]:.2f}\n'
          f'📥 <b>Total Deposited:</b> ₹{u[8]:.2f}\n📅 <b>Joined:</b> {html.escape(str(u[9] or "N/A"))}\n'
          f'👑 <b>Reseller:</b> {"YES" if u[11] else "NO"}\n🏷️ <b>Discount:</b> {u[12]:.0f}%\n🔗 <b>Referral Code:</b> <code>{get_user_referral_code(uid) or "Not created"}</code>\n\n🛒 <b>Purchased Keys / Orders:</b>\n')
    if orders:
        for o in orders[:50]:
            text += f'• <code>{o[0]}</code> · {html.escape(str(o[1]))} · {html.escape(str(o[2]))} · ₹{o[3]:.2f} · {html.escape(str(o[5]))}\n  🔑 <code>{html.escape(str(o[4]))}</code>\n'
    else: text+='No purchase history.'
    m=types.InlineKeyboardMarkup(); m.add(types.InlineKeyboardButton('🔙 Back',callback_data=f'usys_user_{uid}')); bot.send_message(chat_id,text,parse_mode='HTML',reply_markup=m)

# ---- Bot Settings ----
def _bot_settings_show(chat_id, message_id=None):
    text='🛠️ <b>BOT SETTINGS</b>\n\nControl all user-facing text, Premium emojis, button colors, button emojis, backups and admins from here.'
    m=_admin_category_markup('bot_settings')
    if message_id:
        try: bot.edit_message_text(text,chat_id,message_id,parse_mode='HTML',reply_markup=m); return
        except Exception: pass
    bot.send_message(chat_id,text,parse_mode='HTML',reply_markup=m)

def _show_bot_text_editor(chat_id):
    m=types.InlineKeyboardMarkup()
    items=[
      ('🏠 Main Menu Text','main_menu'),('🛍️ Store Menu Text','store_menu'),('👋 Welcome Message Text + Premium Emoji','welcome_message'),
      ('📱 Verification / First Start Text','verification_required'),('✅ Phone Verified Text','phone_verified'),('👤 My Profile Text','my_profile'),
      ('🛒 Order Summary Text','order_summary'),('📜 Order History Text','order_history'),('🎁 Referral Program Text','referral_program'),
      ('🎬 How To Use Bot Text + Premium Emoji','how_to_use'),('💰 Add Balance Text + Premium Emoji','top_up'),('📲 QR Below Text + Premium Emoji','qr_payment'),
      ('💳 Wallet Payment Text + Premium Emoji','wallet_payment'),('💰 Wallet Balance Success Text + Premium Emoji','wallet_topup_success'),
      ('🇮🇳 UPI Payment Text + Premium Emoji','upi_payment'),('⏳ Payment Expired Text','payment_expired'),('🛍️ Product/Plan Text + Premium Emoji','plan_text_add'),
    ]
    for label,key in items: m.add(types.InlineKeyboardButton(label,callback_data=f'edit_txt_{key}'))
    m.add(types.InlineKeyboardButton('🔙 Bot Settings',callback_data='admin_cat_bot_settings'))
    bot.send_message(chat_id,'✏️ <b>BOT TEXT & PREMIUM EMOJI</b>\n\nEvery editable user-facing text is listed here. Live values such as user name, Telegram ID, balance, amount, order ID, product name, plan and referral link remain automatic.',parse_mode='HTML',reply_markup=m)

def _send_bot_backup(chat_id):
    import zipfile
    zip_path='rg_cheat_shop_bot_backup.zip'
    try:
        with zipfile.ZipFile(zip_path,'w',zipfile.ZIP_DEFLATED) as z:
            z.write('shop_data.db','shop_data.db')
            z.write(os.path.abspath(__file__),os.path.basename(__file__))
        with open(zip_path,'rb') as f: bot.send_document(chat_id,f,caption='💾 <b>Bot Backup</b> — source + database',parse_mode='HTML')
    except Exception as e:
        bot.send_message(chat_id,f'❌ Backup failed: <code>{html.escape(str(e))}</code>',parse_mode='HTML')
    finally:
        try: os.remove(zip_path)
        except Exception: pass

def _show_admin_management(chat_id):
    m=types.InlineKeyboardMarkup()
    for aid in ADMIN_IDS: m.add(types.InlineKeyboardButton(f'👑 Admin {aid}',callback_data=f'admin_manage_one_{aid}'))
    m.add(types.InlineKeyboardButton('➕ Add Admin',callback_data='admin_add_admin'))
    m.add(types.InlineKeyboardButton('🔙 Bot Settings',callback_data='admin_cat_bot_settings'))
    bot.send_message(chat_id,'👑 <b>ADMIN MANAGEMENT</b>\n\nAdd or remove Telegram admins. The built-in owner IDs remain protected.',parse_mode='HTML',reply_markup=m)

def _save_fampay_upi(message):
    if message.from_user.id not in ADMIN_IDS: return
    value=(message.text or '').strip()
    if not value:
        m=bot.send_message(message.chat.id,'❌ UPI ID cannot be empty. Enter again:'); _register_next_step_handler(m,_save_fampay_upi); return
    set_setting('fampay_upi_id',value)
    bot.send_message(message.chat.id,'✅ <b>FamPay UPI ID saved.</b>',parse_mode='HTML',reply_markup=_admin_category_markup('payment'))

def _save_fampay_key(message):
    if message.from_user.id not in ADMIN_IDS: return
    value=(message.text or '').strip()
    if not value:
        m=bot.send_message(message.chat.id,'❌ API Token cannot be empty. Enter again:'); _register_next_step_handler(m,_save_fampay_key); return
    set_setting('fampay_api_key',value)
    bot.send_message(message.chat.id,'✅ <b>FamPay API Token saved.</b>',parse_mode='HTML',reply_markup=_admin_category_markup('payment'))

def _save_paytm_upi(message):
    if message.from_user.id not in ADMIN_IDS: return
    value=(message.text or '').strip()
    if not value:
        m=bot.send_message(message.chat.id,'❌ UPI ID cannot be empty. Enter again:'); _register_next_step_handler(m,_save_paytm_upi); return
    set_setting('paytm_upi_id',value)
    bot.send_message(message.chat.id,'✅ <b>Paytm UPI ID saved.</b>',parse_mode='HTML',reply_markup=_admin_category_markup('payment'))

def _save_paytm_mid(message):
    if message.from_user.id not in ADMIN_IDS: return
    value=(message.text or '').strip()
    if not value:
        m=bot.send_message(message.chat.id,'❌ Merchant ID cannot be empty. Enter again:'); _register_next_step_handler(m,_save_paytm_mid); return
    set_setting('paytm_merchant_id',value)
    bot.send_message(message.chat.id,'✅ <b>Paytm Merchant ID saved.</b>',parse_mode='HTML',reply_markup=_admin_category_markup('payment'))

def _save_new_admin_id(message):
    if message.from_user.id not in ADMIN_IDS: return
    try: aid=int((message.text or '').strip())
    except Exception:
        m=bot.send_message(message.chat.id,'❌ Invalid Telegram ID. Enter numeric ID:'); _register_next_step_handler(m,_save_new_admin_id); return
    if aid not in ADMIN_IDS: ADMIN_IDS.append(aid); _persist_admin_ids()
    bot.send_message(message.chat.id,f'✅ <b>Admin added:</b> <code>{aid}</code>',parse_mode='HTML',reply_markup=_admin_category_markup('bot_settings'))
...
def handle_menu_click(call):
    ...
    # Banned users cannot use inline callbacks. Admins are always allowed.
    if user_id not in ADMIN_IDS and is_user_banned(user_id):
        try: bot.answer_callback_query(call.id, '🚫 Your account is banned.', show_alert=True)
        except Exception: pass
        return
    
    if call.data == 'admin_main':
        if user_id in ADMIN_IDS: admin_panel(call.message)

    elif call.data == 'admin_cat_setup':
        if user_id in ADMIN_IDS: _all_settings_show(call.message.chat.id,call.message.message_id)

    elif call.data == 'admin_cat_api':
        if user_id in ADMIN_IDS: bot.send_message(call.message.chat.id,'🌐 <b>API & SERVER SYSTEMS</b>\n\nOnly the new server API flow is exposed here.',parse_mode='HTML',reply_markup=_admin_category_markup('api'))

    elif call.data == 'admin_cat_users':
        if user_id in ADMIN_IDS: _user_system_show(call.message.chat.id,call.message.message_id)

    elif call.data == 'admin_cat_payment':
        if user_id in ADMIN_IDS: bot.send_message(call.message.chat.id,'💳 <b>BOT PAYMENT SYSTEM</b>\n\nConfigure the payment receivers used by this bot.',parse_mode='HTML',reply_markup=_admin_category_markup('payment'))

    elif call.data == 'admin_cat_bot_settings':
        if user_id in ADMIN_IDS: _bot_settings_show(call.message.chat.id,call.message.message_id)

    elif call.data == 'all_settings_open':
        if user_id in ADMIN_IDS: _all_settings_show(call.message.chat.id,call.message.message_id)

    elif call.data == 'allset_add_hack':
        if user_id in ADMIN_IDS: _start_all_settings_add_hack(call.message)

    elif call.data.startswith('allset_hack_'):
        if user_id in ADMIN_IDS:
            name=_all_settings_hack_by_hash(call.data.replace('allset_hack_','',1))
            if name: _all_settings_plan_prompt(call.message.chat.id,name)

    elif call.data.startswith('allset_add_plan_'):
        if user_id in ADMIN_IDS:
            name=_all_settings_hack_by_hash(call.data.replace('allset_add_plan_','',1))
            if name: _start_all_settings_add_plan(call.message,name)

    elif call.data.startswith('allset_plan_'):
        if user_id in ADMIN_IDS:
            try: _all_settings_plan_edit_menu(call.message.chat.id,int(call.data.replace('allset_plan_','',1)))
            except Exception: pass

    elif call.data.startswith('allset_edit_hack_'):
        if user_id in ADMIN_IDS:
            name=_all_settings_hack_by_hash(call.data.replace('allset_edit_hack_','',1))
            if name: _all_settings_edit_hack_menu(call.message.chat.id,name)

    elif call.data.startswith('allset_rename_'):
        if user_id in ADMIN_IDS:
            name=_all_settings_hack_by_hash(call.data.replace('allset_rename_','',1))
            if name:
                admin_temp_data[user_id]={'old_name':name}
                m=bot.send_message(call.message.chat.id,f'✏️ <b>Enter new name for:</b> {html.escape(name)}')
                _register_next_step_handler(m,_save_all_settings_rename)

    elif call.data.startswith('allset_edit_days_'):
        if user_id in ADMIN_IDS:
            pid=int(call.data.replace('allset_edit_days_','',1)); admin_temp_data[user_id]={'plan_id':pid}; m=bot.send_message(call.message.chat.id,'📅 <b>Enter new Plan / Days:</b>'); _register_next_step_handler(m,lambda mm:_save_all_settings_plan_field(mm,'days'))

    elif call.data.startswith('allset_edit_user_'):
        if user_id in ADMIN_IDS:
            pid=int(call.data.replace('allset_edit_user_','',1)); admin_temp_data[user_id]={'plan_id':pid}; m=bot.send_message(call.message.chat.id,'💰 <b>Enter new User Price:</b>'); _register_next_step_handler(m,lambda mm:_save_all_settings_plan_field(mm,'user'))

    elif call.data.startswith('allset_edit_resell_'):
        if user_id in ADMIN_IDS:
            pid=int(call.data.replace('allset_edit_resell_','',1)); admin_temp_data[user_id]={'plan_id':pid}; m=bot.send_message(call.message.chat.id,'💎 <b>Enter new Reseller Price:</b>'); _register_next_step_handler(m,lambda mm:_save_all_settings_plan_field(mm,'resell'))

    elif call.data == 'user_system_open':
        if user_id in ADMIN_IDS: _user_system_show(call.message.chat.id,call.message.message_id)

    elif call.data.startswith('usys_user_'):
        if user_id in ADMIN_IDS: _user_detail_show(call.message.chat.id,int(call.data.replace('usys_user_','',1)))

    elif call.data.startswith('usys_ban_'):
        if user_id in ADMIN_IDS:
            parts=call.data.split('_'); uid=int(parts[2]); banned=int(parts[3]); set_user_banned(uid,bool(banned)); _user_detail_show(call.message.chat.id,uid)
            try: bot.send_message(uid,'🚫 <b>Your account has been banned by the administrator.</b>' if banned else '✅ <b>Your account has been unbanned.</b>',parse_mode='HTML')
            except Exception: pass

    elif call.data.startswith('usys_reseller_'):
        if user_id in ADMIN_IDS:
            parts=call.data.split('_'); uid=int(parts[2]); status=int(parts[3]); set_reseller_status(uid,status); _user_detail_show(call.message.chat.id,uid)

    elif call.data.startswith('usys_discount_'):
        if user_id in ADMIN_IDS:
            uid=int(call.data.replace('usys_discount_','',1)); admin_temp_data[user_id]={'target_uid':uid}; m=bot.send_message(call.message.chat.id,'🏷️ <b>Enter discount percentage (0-100):</b>'); _register_next_step_handler(m,step_get_user_discount_percent)

    elif call.data.startswith('usys_refcode_'):
        if user_id in ADMIN_IDS:
            uid=int(call.data.replace('usys_refcode_','',1)); code=create_or_refresh_user_referral_code(uid,refresh=True); bot.send_message(call.message.chat.id,f'🔗 <b>Referral Code Created</b>\n\n👤 User: <code>{uid}</code>\n🎟️ Code: <code>{code}</code>',parse_mode='HTML',reply_markup=_user_detail_markup(uid))

    elif call.data.startswith('usys_track_'):
        if user_id in ADMIN_IDS: _user_tracking_show(call.message.chat.id,int(call.data.replace('usys_track_','',1)))

    elif call.data == 'payment_fampay':
        if user_id in ADMIN_IDS:
            current_upi=get_fampay_upi(); current_key=get_fampay_api_key()
            m=types.InlineKeyboardMarkup(); m.add(types.InlineKeyboardButton('✏️ Set FamPay UPI ID',callback_data='payment_fampay_upi')); m.add(types.InlineKeyboardButton('🔑 Set FamPay API Token',callback_data='payment_fampay_key')); m.add(types.InlineKeyboardButton('🔙 Back',callback_data='admin_cat_payment'))
            bot.send_message(call.message.chat.id,f'💳 <b>FAMPAY AUTO PAYMENT</b>\n\nUPI: <code>{html.escape(current_upi)}</code>\nAPI Token: <code>{html.escape(current_key[:8])}••••••</code>',parse_mode='HTML',reply_markup=m)

    elif call.data == 'payment_fampay_upi':
        if user_id in ADMIN_IDS:
            m=bot.send_message(call.message.chat.id,'💳 <b>Send FamPay UPI ID:</b>\n\n/cancel to cancel.'); _register_next_step_handler(m,_save_fampay_upi)

    elif call.data == 'payment_fampay_key':
        if user_id in ADMIN_IDS:
            m=bot.send_message(call.message.chat.id,'🔑 <b>Send FamPay API Token:</b>\n\n/cancel to cancel.'); _register_next_step_handler(m,_save_fampay_key)

    elif call.data == 'payment_paytm':
        if user_id in ADMIN_IDS:
            m=types.InlineKeyboardMarkup(); m.add(types.InlineKeyboardButton('✏️ Set Paytm UPI ID',callback_data='payment_paytm_upi')); m.add(types.InlineKeyboardButton('🆔 Set Merchant ID',callback_data='payment_paytm_mid')); m.add(types.InlineKeyboardButton('🔙 Back',callback_data='admin_cat_payment'))
            bot.send_message(call.message.chat.id,f'🇮🇳 <b>PAYTM UPI AUTO PAYMENT</b>\n\nUPI: <code>{html.escape(get_paytm_upi() or "Not set")}</code>\nMerchant ID: <code>{html.escape(get_paytm_merchant_id() or "Not set")}</code>\n\n⚠️ Auto-verification also requires Paytm server-side signing credentials; this source does not currently contain that credential/API implementation.',parse_mode='HTML',reply_markup=m)

    elif call.data == 'payment_paytm_upi':
        if user_id in ADMIN_IDS:
            m=bot.send_message(call.message.chat.id,'🇮🇳 <b>Send Paytm UPI ID:</b>'); _register_next_step_handler(m,_save_paytm_upi)

    elif call.data == 'payment_paytm_mid':
        if user_id in ADMIN_IDS:
            m=bot.send_message(call.message.chat.id,'🆔 <b>Send Paytm Merchant ID:</b>'); _register_next_step_handler(m,_save_paytm_mid)

    elif call.data == 'bot_settings_texts':
        if user_id in ADMIN_IDS: _show_bot_text_editor(call.message.chat.id)

    elif call.data == 'bot_settings_colors':
        if user_id in ADMIN_IDS: show_user_panel_button_color_catalog(call.message.chat.id,message_id=call.message.message_id)

    elif call.data == 'bot_settings_emojis':
        if user_id in ADMIN_IDS: show_user_panel_button_emoji_catalog(call.message.chat.id,message_id=call.message.message_id)

    elif call.data == 'bot_settings_backup':
        if user_id in ADMIN_IDS: _send_bot_backup(call.message.chat.id)

    elif call.data == 'bot_settings_admins':
        if user_id in ADMIN_IDS: _show_admin_management(call.message.chat.id)

    elif call.data == 'admin_add_admin':
        if user_id in ADMIN_IDS:
            m=bot.send_message(call.message.chat.id,'➕ <b>Enter Telegram User ID to make Admin:</b>'); _register_next_step_handler(m,_save_new_admin_id)

    elif call.data.startswith('admin_manage_one_'):
        if user_id in ADMIN_IDS:
            aid=int(call.data.replace('admin_manage_one_','',1)); m=types.InlineKeyboardMarkup(); m.add(types.InlineKeyboardButton('❌ Remove Admin',callback_data=f'admin_remove_one_{aid}')); m.add(types.InlineKeyboardButton('🔙 Back',callback_data='bot_settings_admins')); bot.send_message(call.message.chat.id,f'👑 <b>Admin:</b> <code>{aid}</code>',parse_mode='HTML',reply_markup=m)

    elif call.data.startswith('admin_remove_one_'):
        if user_id in ADMIN_IDS:
            aid=int(call.data.replace('admin_remove_one_','',1))
            if aid in DEFAULT_ADMIN_IDS or len(ADMIN_IDS)<=1:
                bot.answer_callback_query(call.id,'Protected admin cannot be removed.',show_alert=True)
            else:
                ADMIN_IDS.remove(aid); _persist_admin_ids(); _show_admin_management(call.message.chat.id)
    ...
    if call.data == "open_store":
        ...
        if req_dev_id == 1:
            ...
            _register_next_step_handler(msg, step_get_device_id_input)
        else:
    ...
    # ROBUST WALLET PAYMENT CALLBACK
        ...
        _delete_message_safely(call.message.chat.id, call.message.message_id)
        process_wallet_checkout(call.message.chat.id, user_id, product_id, device_id=device_id)

    elif call.data == "admin_api_system":
        if user_id in ADMIN_IDS:
            markup=types.InlineKeyboardMarkup(); markup.add(types.InlineKeyboardButton("🆕 New Server API System",callback_data="new_server_api")); markup.add(types.InlineKeyboardButton("🌐 Add & Connect Server API",callback_data="admin_server_api_mgmt")); markup.add(types.InlineKeyboardButton("⚙️ Hack API & Server Manager",callback_data="admin_hack_server_manager")); markup.add(types.InlineKeyboardButton("🔗 Map Server Hack ID & Duration",callback_data="admin_map_server_id")); markup.add(types.InlineKeyboardButton("🔙 Main Admin Panel",callback_data="admin_main")); bot.send_message(call.message.chat.id,f"🌐 <b>API & SERVER SYSTEMS</b>\n\nNew Server: <b>{_new_server_status_text()}</b>",parse_mode="HTML",reply_markup=markup)

    elif call.data == "api_srv_add":
        if user_id in ADMIN_IDS:
            admin_temp_data[user_id] = {}
            m = bot.send_message(call.message.chat.id, "🔑 <b>Enter New API Server API Key:</b>", parse_mode="HTML")
            _register_next_step_handler(m, step_save_api_server_key)

    elif call.data.startswith("api_srv_") and call.data[8:].isdigit():
        if user_id in ADMIN_IDS:
            sid = int(call.data[8:])
            row = _api_server_row(sid)
            if not row:
                bot.answer_callback_query(call.id, "Server not found.", show_alert=True)
                return
            _, name, api_key, url, enabled, created_at, last_connected_at, account_name, account_username, account_user_id = row
            status = "🟢 CONNECTED" if enabled else "🔴 NOT CONNECTED"
            connected_line = f"🕒 <b>Connected:</b> {html.escape(str(last_connected_at))}\n" if last_connected_at else ""
            identity_parts = []
            if account_name:
                identity_parts.append(f"👤 {html.escape(str(account_name))}")
            if account_username:
                identity_parts.append(f"@{html.escape(str(account_username).lstrip('@'))}")
            if account_user_id:
                identity_parts.append(f"ID {html.escape(str(account_user_id))}")
            identity_line = f"👤 <b>Reseller:</b> {' · '.join(identity_parts)}\n" if identity_parts else ""
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("⚙️ Configure Hacks", callback_data=f"api_hacks_{sid}"))
            markup.add(types.InlineKeyboardButton("🔍 Check Connection", callback_data=f"api_check_{sid}"))
            markup.add(types.InlineKeyboardButton("🔙 API Servers", callback_data="new_server_api"))
            bot.send_message(
                ...
                f"🔗 <b>Server:</b> <code>{html.escape(url)}</code>\n"
                f"📡 <b>Status:</b> {status}\n"
                f"{identity_line}"
                f"{connected_line}"
                f"🔐 <b>API Key:</b> <code>{html.escape(str(api_key)[:8])}••••••</code>\n\n"
                "Use Configure Hacks to assign Product IDs and plan Valid IDs.",
                parse_mode="HTML", reply_markup=markup
            )

    elif call.data.startswith("api_check_"):
        if user_id in ADMIN_IDS:
            try:
                sid = int(call.data.replace("api_check_", "", 1))
            except ValueError:
                return
            ok, detail = _api_server_test_connection(sid)
            bot.answer_callback_query(call.id, "Connected Successfully!" if ok else "Not Connect", show_alert=True)
            row = _api_server_row(sid)
            if row:
                status = "🟢 <b>Connected Successfully!</b>" if ok else "🔴 <b>Not Connect</b>"
                identity_parts = []
                if len(row) > 7 and row[7]:
                    identity_parts.append(html.escape(str(row[7])))
                if len(row) > 8 and row[8]:
                    identity_parts.append('@' + html.escape(str(row[8]).lstrip('@')))
                if len(row) > 9 and row[9]:
                    identity_parts.append('ID ' + html.escape(str(row[9])))
                identity_line = f"👤 <b>Reseller:</b> {' · '.join(identity_parts)}\n" if identity_parts else ""
                connected_line = f"🕒 <b>Connected:</b> {html.escape(str(row[6]))}\n" if len(row) > 6 and row[6] else ""
                bot.send_message(
                    call.message.chat.id,
                    f"{status}\n\n🌐 <b>{html.escape(row[1])}</b>\n"
                    f"🔗 <code>{html.escape(row[3])}</code>\n"
                    f"{identity_line}{connected_line}",
                    parse_mode="HTML"
                )

    elif call.data.startswith("api_hacks_"):
        if user_id in ADMIN_IDS:
            try:
                sid = int(call.data.replace("api_hacks_", "", 1))
            except ValueError:
                return
            row = _api_server_row(sid)
            if not row:
            ...
            bot.send_message(
                call.message.chat.id,
                f"🎮 <b>HACK LIST — {html.escape(row[1])}</b>\n\n"
                "Select a hack. Then set its Product ID and each plan's Valid ID.",
                parse_mode="HTML", reply_markup=_api_server_hacks_markup(sid)
            )

    elif call.data.startswith("api_hack_"):
        if user_id in ADMIN_IDS:
            ...
            try:
                ...
                return
            product_name = _api_server_hack_by_hash(parts[3])
            if not product_name:
                return
            row = _api_server_row(sid)
            if not row:
                return
            pid = _api_server_product_id(sid, product_name)
            bot.send_message(
                call.message.chat.id,
                f"🎮 <b>{html.escape(product_name)}</b>\n\n"
                f"🆕 <b>Server:</b> {html.escape(row[1])}\n"
                f"🆔 <b>Product ID:</b> <code>{html.escape(pid or 'Not Set')}</code>\n\n"
                "Select a plan to enter its Valid ID.",
                parse_mode="HTML", reply_markup=_api_server_plans_markup(sid, product_name)
            )

    elif call.data.startswith("api_pid_"):
        if user_id in ADMIN_IDS:
            parts = call.data.split("_", 3)
            if len(parts) != 4:
                return
            ...
            if not product_name:
                return
            admin_temp_data[user_id] = {"api_server_id": sid, "api_server_product": product_name}
            m = bot.send_message(
                call.message.chat.id,
                f"🆔 <b>Enter Product ID for {html.escape(product_name)}:</b>",
                parse_mode="HTML"
            )
            _register_next_step_handler(m, step_save_multi_api_product_id)

    elif call.data.startswith("api_plan_"):
        if user_id in ADMIN_IDS:
            ...
            if not product_name:
                return
            plans = {int(x[0]): x for x in get_product_plans(product_name)}
            plan = plans.get(plan_id)
            if not plan:
                return
            admin_temp_data[user_id] = {
                "api_server_id": sid,
                "api_server_product": product_name,
                "api_server_plan_id": plan_id,
                "api_server_plan_days": str(plan[2])
            }
            m = bot.send_message(
                call.message.chat.id,
                f"🆔 <b>Enter Valid ID for {html.escape(str(plan[2]))}:</b>",
                parse_mode="HTML"
            )
            _register_next_step_handler(m, step_save_multi_api_valid_id)

    elif call.data == "new_server_api":
        if user_id in ADMIN_IDS:
            _show_api_server_list(call.message.chat.id, message_id=None)

    elif call.data.startswith("new_server_hack_"):
        if user_id in ADMIN_IDS:
            product_name=_new_server_hack_by_hash(call.data.replace("new_server_hack_",'',1));
            if not product_name: return
            cfg=_new_server_config(product_name); pid=cfg[3] if cfg else ''
            markup=_new_server_plans_markup(product_name); markup.add(types.InlineKeyboardButton("🆔 Set / Change Product ID",callback_data=f"new_server_set_pid_{_ui_hash(product_name)}")); markup.add(types.InlineKeyboardButton("🔙 Hack List",callback_data="new_server_api"))
            bot.send_message(call.message.chat.id,f"🎮 <b>{html.escape(product_name)}</b>\n\n🌐 <b>New Server:</b> <code>{html.escape(NEW_SERVER_API_URL)}</code>\n🆔 <b>Product ID:</b> <code>{html.escape(pid or 'Not Set')}</code>\n\nTap a plan to set its Valid ID.",parse_mode="HTML",reply_markup=markup)

    elif call.data.startswith("new_server_set_pid_"):
        if user_id in ADMIN_IDS:
            product_name=_new_server_hack_by_hash(call.data.replace("new_server_set_pid_",'',1));
            if not product_name: return
            admin_temp_data[user_id]={'new_server_product':product_name}; m=bot.send_message(call.message.chat.id,f"🆔 <b>Enter New Server Product ID for {html.escape(product_name)}:</b>\n\nThe ID is stored exactly as entered.",parse_mode="HTML"); _register_next_step_handler(m,step_save_new_server_product_id)

    elif call.data.startswith("new_server_plan_"):
        if user_id in ADMIN_IDS:
            raw=call.data.replace("new_server_plan_",'',1); pieces=raw.rsplit('_',1)
            if len(pieces)!=2: return
            product_name=_new_server_hack_by_hash(pieces[0])
            try: plan_id=int(pieces[1])
            except ValueError: return
            if not product_name: return
            plans={int(x[0]):x for x in get_product_plans(product_name)}; plan=plans.get(plan_id)
            if not plan: return
            admin_temp_data[user_id]={'new_server_product':product_name,'new_server_plan_id':plan_id,'new_server_plan_days':str(plan[2])}; m=bot.send_message(call.message.chat.id,f"🆔 <b>Enter New Server Valid ID for {html.escape(str(plan[2]))}:</b>\n\nThe Valid ID is stored exactly as entered.",parse_mode="HTML"); _register_next_step_handler(m,step_save_new_server_valid_id)
        ...
        if user_id in ADMIN_IDS:
            ...
            msg = bot.send_message(
                ...
                "You may use any text/formatting and Premium emojis. Send only ONE sample Validity/Stock/Price block; its values are placeholders. The bot will automatically inject every real plan, INR price, Manual/Auto-API/Out of Stock status and discount.\n\n"
                 "✨ The design can be compact like a card (for example: a DAY line followed by Stock and Price) or detailed with tree/box lines. Both styles are supported. Keep the words Validity, Stock and Price in the sample so the live fields can be detected.\n\n"
            ...
            _register_next_step_handler(msg, step_save_hack_price_ui_title)
            bot.answer_callback_query(call.id)
        ...
        if user_id in ADMIN_IDS:
            ...
            _register_next_step_handler(msg, step_save_product_plan_text)
            bot.answer_callback_query(call.id)
        ...
        if user_id in ADMIN_IDS:
            ...
            _register_next_step_handler(msg, step_save_global_price_list_template)
            bot.answer_callback_query(call.id)
        ...
        if user_id in ADMIN_IDS:
            ...
            _register_next_step_handler(msg, step_save_premium_emoji)
            bot.answer_callback_query(call.id)
        ...
        if user_id in ADMIN_IDS:
            ...
            _register_next_step_handler(msg, step_save_custom_btn)
        ...
        if user_id in ADMIN_IDS:
            ...
            markup.add(types.InlineKeyboardButton("👋 Welcome Message Text + Premium Emoji", callback_data="edit_txt_welcome_message"))
        ...
        if user_id in ADMIN_IDS:
            ...
            if t_key == "add_balance":
                ...
                t_key = "top_up_qr"
            if t_key == "welcome_message":
                current_welcome=get_welcome_message_template()
                msg=bot.send_message(call.message.chat.id,"👋 <b>WELCOME MESSAGE TEXT + PREMIUM EMOJI</b>\n\nSend the complete Welcome Message. Premium custom emojis are supported.\n\nLive placeholders: <code>{name}</code>, <code>{username}</code>, <code>{user_id}</code>, <code>{telegram_id}</code>, <code>{date}</code>.\n\n📌 <b>Current design:</b>\n"+current_welcome[:3000],parse_mode="HTML")
                _register_next_step_handler(msg,step_save_welcome_message); return
            if t_key == "price_expiry":
                ...
                if t_key == "wallet_topup_success":
                    ...
                    _register_next_step_handler(msg, step_save_wallet_topup_success_template)
                    return
                if t_key == "wallet_payment":
                    ...
                    msg = bot.send_message(call.message.chat.id, prompt, parse_mode="HTML")
                    _register_next_step_handler(msg, step_save_wallet_payment_template)
                    return
                if t_key == "upi_payment":
                    ...
                    msg = bot.send_message(call.message.chat.id, prompt, parse_mode="HTML")
                    _register_next_step_handler(msg, step_save_upi_payment_template)
                    return
                if t_key == "qr_payment":
                    prompt = (
                ...
                _register_next_step_handler(msg, step_save_custom_text)
        ...
        if user_id in ADMIN_IDS:
            ...
            _register_next_step_handler(msg, step_set_tg_group_link)
        ...
        if user_id in ADMIN_IDS:
            ...
            _register_next_step_handler(msg, step_add_hack_name)
        ...
        if user_id in ADMIN_IDS:
            ...
            _register_next_step_handler(msg, step_get_add_day_to_hack)

    elif call.data == "admin_hack_server_manager":
        if user_id in ADMIN_IDS:
            if not get_unique_products(): bot.send_message(call.message.chat.id,"❌ No hacks/products found."); return
            bot.send_message(call.message.chat.id,"⚙️ <b>HACK API & SERVER MANAGER</b>\n\nSelect a hack. Its API/Server/Product ID and plan-by-plan Valid IDs are saved separately.",parse_mode="HTML",reply_markup=_server_manager_hacks_markup())
    elif call.data.startswith("hack_server_select_"):
        if user_id in ADMIN_IDS:
            p_name=call.data.replace("hack_server_select_","",1)
            if p_name not in get_unique_products(): return
            admin_temp_data[user_id]={'hack_server_product':p_name}; msg=bot.send_message(call.message.chat.id,f"🎮 <b>Selected Hack:</b> {html.escape(p_name)}\n\n🔑 <b>Enter API Key:</b>",parse_mode="HTML"); _register_next_step_handler(msg,step_save_hack_server_api_key)
        ...
        if user_id in ADMIN_IDS:
            ...
            _register_next_step_handler(msg, step_get_server_token)
        ...
        if user_id in ADMIN_IDS:
            ...
            _register_next_step_handler(msg, step_get_remote_pid)
        ...
        if user_id in ADMIN_IDS:
            ...
            if prod:
                ...
                _register_next_step_handler(msg, save_key_step)
        ...
        if user_id in ADMIN_IDS:
            ...
            _register_next_step_handler(msg, step_get_broadcast_media)
