...
TOKEN = '8941581162:AAEAce69CtRBgwGifhMsaByEm07o7aXfg04'
...
def _extract_api_key_value(data):
    """Extract a delivered key from common JSON envelopes returned by reseller APIs."""
    if isinstance(data, dict):
        direct_fields = (
            'key', 'license', 'license_key', 'serial', 'code', 'key_value',
            'licence', 'access_key', 'product_key', 'generated_key',
            'generatedKey', 'key_value_text', 'key_text', 'licenseCode'
        )
        for field in direct_fields:
            ...
            if value not in (None, '') and not isinstance(value, (dict, list)):
                ...
                if len(text) > 3:
                    return text
        for field in (
            'data', 'result', 'response', 'order', 'delivery', 'purchase',
            'sale', 'payload', 'key_data', 'keyData', 'keys', 'items', 'details'
        ):
            value = data.get(field)
...
def _extract_api_order_id(data):
    """Extract the provider order id without confusing it with a user/product id."""
    if isinstance(data, dict):
        for field in (
            'order_id', 'orderId', 'purchase_id', 'purchaseId',
            'rap_id', 'rapId', 'reference_id', 'referenceId',
            'transaction_id', 'transactionId', 'sale_id', 'saleId'
        ):
            value = data.get(field)
            if value not in (None, '') and not isinstance(value, (dict, list)):
                text = str(value).strip()
                if len(text) > 2:
                    return text
        for field in ('data', 'result', 'response', 'order', 'delivery'):
            value = data.get(field)
...
def _new_api_get_order_key(server_url, api_key, order_id, max_wait_seconds=30):
    """Retrieve a key for an already-created provider order.

    The important safety rule is: once generate-key has created an order, this
    function only READS the order. It never creates another sale.
    """
    ...
    headers = {
        ...
        'User-Agent': 'RG-Cheat-Shop-Bot/1.3',
    }
    deadline = time.time() + max(1, int(max_wait_seconds or 30))
    attempt = 0
    last_detail = ''

    # The documented route is /orders/{id}. The query form is a read-only
    # compatibility fallback for deployments that expose the same resource that way.
    routes = [
        f"{server_url}/api/v1/orders/{urllib.parse.quote(order_id, safe='')}",
        f"{server_url}/api/v1/order/{urllib.parse.quote(order_id, safe='')}",
        f"{server_url}/api/v1/orders?order_id={urllib.parse.quote(order_id, safe='')}",
    ]
    ...
    while time.time() <= deadline:
        ...
        for url in routes:
            try:
                ...
                print(f"DEBUG NEW API ORDER LOOKUP attempt={attempt} order={order_id} status={response.status_code}: {raw[:1500]}")

                data = _api_server_parse_json(response)
                key_value = _extract_api_key_value(data)
                if not key_value and raw and not raw.startswith('{') and not raw.startswith('[') and len(raw) < 500:
                    # Accept only an explicitly key-labelled plain response.
                    m = re.search(r'(?i)(?:^|\b)(?:key|license(?:_key)?|code)\s*[:=]\s*([^\r\n]+)', raw)
                    if m:
                        key_value = m.group(1).strip().strip('"\'')

                if response.status_code == 200 and key_value:
                    print(f"DEBUG NEW API ORDER LOOKUP SUCCESS order={order_id}")
                    return key_value

                last_detail = f"HTTP {response.status_code}: {raw[:500]}"
                if response.status_code in (401, 403) or response.status_code >= 500:
                    break
            except Exception as exc:
                last_detail = str(exc)[:500]
                print(f"DEBUG NEW API ORDER LOOKUP EXCEPTION order={order_id}: {last_detail}")
...
def _new_api_generate_key(server_url, api_key, product_id, valid_id, duration=None, android_id=None, purchase_ref=None):
    """Call REST v1 generate-key and reliably retrieve the delivered key.

    Important provider behavior: a successful POST /api/v1/generate-key may return
    an order_id before the key is available in that response.  The provider then
    exposes the final key through GET /api/v1/orders/{order_id}.  We therefore
    capture order_id immediately and perform a short lookup/poll before declaring
    delivery failed.  This is also why we must NOT blindly retry a successful
    purchase with another payload: that could create a second sale.
    """
    server_url = _normalize_api_server_url(server_url)
    api_key = str(api_key or '').strip()
    ...
    if not server_url or not api_key or not product_id or not valid_id:
        print("DEBUG NEW API: missing server/product/valid mapping")
        return None

    purchase_ref = str(purchase_ref or '').strip()
    if not purchase_ref:
        purchase_ref = f"{time.time_ns()}-{random.randint(100000, 999999)}"
    idempotency_source = '|'.join((server_url, product_id, valid_id, android_id, duration, purchase_ref))
    idempotency_key = hashlib.sha256(idempotency_source.encode('utf-8')).hexdigest()[:48]
    headers = {
        'Authorization': f'Bearer {api_key}',
        'Accept': 'application/json',
        'Content-Type': 'application/json',
        'Idempotency-Key': idempotency_key,
        'User-Agent': 'RG-Cheat-Shop-Bot/1.2',
    }

    # The configured UI field is explicitly called Valid ID, so use that
    # contract first. Never spray multiple buy requests after a successful 2xx
    # response: some providers ignore Idempotency-Key and that can charge twice.
    payloads = []
    base = {'product_id': product_id, 'valid_id': valid_id}
    if duration:
        payloads.append({**base, 'duration': duration})
    if android_id:
        payloads.append({**base, 'android_id': android_id})
        payloads.append({**base, 'device_id': android_id})
    payloads.append(base)

    # Numeric IDs are only schema fallbacks, and are attempted only after an
    # explicit validation error (never after a successful/accepted purchase).
    numeric_payloads = []
    try:
        product_int = int(product_id)
        valid_int = int(valid_id)
    except (TypeError, ValueError):
        product_int = valid_int = None
    if product_int is not None and valid_int is not None:
        nbase = {'product_id': product_int, 'valid_id': valid_int}
        if duration:
            numeric_payloads.append({**nbase, 'duration': duration})
        if android_id:
            numeric_payloads.append({**nbase, 'android_id': android_id})
        numeric_payloads.append(nbase)
    ...
    for payload in payloads:
        ...
        try:
            response = HTTP_SESSION.post(
                f"{server_url}/api/v1/generate-key",
                json=payload,
                headers=headers,
                timeout=25,
            )
            raw = response.text.strip()
            print(f"DEBUG NEW API GENERATE-KEY payload={marker} status={response.status_code}: {raw[:1500]}")

            data = _api_server_parse_json(response)
            key_value = _extract_api_key_value(data)
            order_id = _extract_api_order_id(data)

            if not key_value and raw and not raw.startswith('{') and not raw.startswith('[') and len(raw) < 500:
                key_value = raw

            # Normal synchronous response: key is already present.
            if response.status_code in (200, 201) and key_value:
                print(f"DEBUG NEW API GENERATE-KEY SUCCESS using payload: {marker}")
                return key_value

            # Critical fix: the provider may confirm the sale with order_id first,
            # then expose the generated key through GET /orders/{order_id}.
            if response.status_code in (200, 201, 202, 409) and order_id:
                print(f"DEBUG NEW API GENERATE-KEY ORDER CREATED order={order_id}; looking up delivered key")
                key_value = _new_api_get_order_key(server_url, api_key, order_id, max_wait_seconds=15)
                if key_value:
                    return key_value
                # The purchase already exists. NEVER submit another purchase payload.
                print(f"DEBUG NEW API: order {order_id} exists but key lookup did not return a key")
                return None
            ...
            # A 2xx response means the provider accepted the request. If it did not
            # include a key/order reference, DO NOT submit another purchase request.
            if 200 <= response.status_code < 300:
                print('DEBUG NEW API: accepted response without key/order id; stopping to prevent duplicate charge')
                return None

            # Only explicit client-side validation errors are safe for a schema
            # fallback. The fallback still reuses the exact same idempotency key.
            if response.status_code in (400, 404, 422) and numeric_payloads:
                payloads.extend(numeric_payloads)
                numeric_payloads = []
            continue
        except Exception as exc:
            last_detail = str(exc)[:500]
            print(f"DEBUG NEW API GENERATE-KEY EXCEPTION: {last_detail}")
            break

    print(f"DEBUG NEW API GENERATE-KEY FAILED: {last_detail}")
    return None
