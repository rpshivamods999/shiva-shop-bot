...
#     btn_hack_server_manager = types.InlineKeyboardButton("⚙️ Hack API & Server Manager", callback_data="admin_hack_server_manager")
...
#     markup.add(btn_hack_server_manager)
...
def admin_panel(message):
    ...
    markup=types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("⚡ Setup & Product Management",callback_data="admin_cat_setup"))
    markup.add(types.InlineKeyboardButton("🌐 API & Server Systems",callback_data="admin_cat_api"))
    markup.add(types.InlineKeyboardButton("✨ Premium Emoji & Button UI",callback_data="admin_cat_emoji"))
    markup.add(types.InlineKeyboardButton("📝 Premium Text & Price List",callback_data="admin_cat_text"))
    markup.add(types.InlineKeyboardButton("👥 Users, Reseller & Discounts",callback_data="admin_cat_users"))
    markup.add(types.InlineKeyboardButton("💰 Balance & Payments",callback_data="admin_cat_money"))
    markup.add(types.InlineKeyboardButton("📢 Broadcast, Guide & Tracking",callback_data="admin_cat_tools"))
    markup.add(types.InlineKeyboardButton("⚙️ Bot / Database Tools",callback_data="admin_cat_system"))
    bot.send_message(message.chat.id,"🛠️ <b>ADMIN CONTROL PANEL</b>\n\nChoose a category below. All existing admin controls remain available inside their sections.",parse_mode="HTML",reply_markup=markup)

def _admin_category_markup(category):
    groups={
      'setup':[("⚡ Quick Admin Setup (One-Msg Hack Setup)","admin_quick_setup"),("🔄 Reorder Hacks List Sequence","admin_reorder_hacks"),("📦 Stock On/Off (Out of Stock System)","admin_stock_mgmt"),("📱 Select Device ID Hack","admin_device_id_mgmt"),("✏️ Edit Product Price & Plans","admin_edit_price"),("🔗 Set Telegram Group Link","admin_set_tg_group"),("⚙️ Manage Hack & Days (Add/Remove)","admin_hack_day_mgmt"),("🔑 Add Manual VIP Keys (FIFO)","admin_add_manual_key")],
      'api':[("🆕 New Server API System","new_server_api"),("🌐 Add & Connect Server API","admin_server_api_mgmt"),("⚙️ Hack API & Server Manager","admin_hack_server_manager"),("🔗 Map Server Hack ID & Duration","admin_map_server_id")],
      'emoji':[("🎨 All Button Color","admin_button_colors"),("✏️ User Panel Names & Emoji","admin_button_cust"),("✨ Premium Emoji Editor","admin_emoji_editor"),("✨ All Button Emoji","admin_all_button_emoji"),("✨ All Product Button Emoji","admin_all_product_emoji"),("💰 All Price List Button Emoji","admin_all_price_emoji"),("💠 All Panel Price & Text + Premium Emoji","admin_all_panel_price_emoji")],
      'text':[("📝 All Product Plan Text + Premium Emoji","admin_plan_text_add"),("🧩 Price List Fix — Emoji & Text","admin_hack_price_ui"),("🟢/🔴 Premium Price List Mode","admin_toggle_price_list_mode"),("✏️ Edit Bot Texts (Text ID)","admin_text_editor")],
      'users':[("👑 Add / Remove Reseller Role","admin_manage_reseller"),("🏷️ Manage Discounts (User/Global/Hack)","admin_manage_discounts"),("🔄 Reset User Completely (By Telegram ID)","admin_user_reset"),("👤 Track Users & Purchased Keys","admin_track_users")],
      'money':[("💰 Manage User Balance","admin_manage_bal"),("⏳ Pending Payments","admin_view_pending_payments")],
      'tools':[("📢 Multimedia Broadcast","admin_broadcast"),("🎬 Dynamic How To Use Bot Link","admin_set_guide")],
      'system':[("🔑 Change Bot API Token","admin_change_bot_token"),("📥 Download Database Backup","admin_db_backup")]
    }
    markup=types.InlineKeyboardMarkup()
...
def step_save_api_server_key(message):
    ...
    if not key:
        ...
        bot.register_next_step_handler(m, step_save_api_server_key)
        return
    ...
    bot.register_next_step_handler(m, step_save_api_server_url)
...
def step_save_multi_api_product_id(message):
    ...
    if not pid:
        ...
        bot.register_next_step_handler(m, step_save_multi_api_product_id)
        return
...
def step_save_multi_api_valid_id(message):
    ...
    if not valid_id:
        ...
        bot.register_next_step_handler(m, step_save_multi_api_valid_id)
        return
...
def step_save_new_server_product_id(message):
    ...
    if _is_command_input(pid):
        ...
        return
    if not pid:
        m=bot.send_message(message.chat.id,"❌ Product ID cannot be empty. Enter the exact Product ID:"); bot.register_next_step_handler(m,step_save_new_server_product_id); return
    if not re.fullmatch(r"\d+", pid):
        m=bot.send_message(message.chat.id,"❌ Invalid Product ID. Enter the numeric Product ID only (example: <code>82</code>).",parse_mode="HTML"); bot.register_next_step_handler(m,step_save_new_server_product_id); return
...
def step_save_new_server_valid_id(message):
    ...
    if _is_command_input(valid_id):
        ...
        return
    if not valid_id:
        m=bot.send_message(message.chat.id,"❌ Valid ID cannot be empty. Enter the exact Valid ID:"); bot.register_next_step_handler(m,step_save_new_server_valid_id); return
    if not re.fullmatch(r"\d+", valid_id):
        m=bot.send_message(message.chat.id,"❌ Invalid Valid ID. Enter the numeric Valid ID only (example: <code>233</code>).",parse_mode="HTML"); bot.register_next_step_handler(m,step_save_new_server_valid_id); return
    cfg=_new_server_config(product_name)
...
def step_save_hack_server_api_key(message):
    ...
    if not d['api_key']:
        m=bot.send_message(message.chat.id,"❌ API Key cannot be empty."); bot.register_next_step_handler(m,step_save_hack_server_api_key); return
    m=bot.send_message(message.chat.id,"🌐 <b>Enter Server URL:</b>",parse_mode="HTML"); bot.register_next_step_handler(m,step_save_hack_server_url)
...
def step_save_hack_server_url(message):
    ...
    if not d.get('hack_server_product') or not url.startswith(('http://','https://')):
        m=bot.send_message(message.chat.id,"❌ Enter a valid Server URL:"); bot.register_next_step_handler(m,step_save_hack_server_url); return
    d['server_url']=url; m=bot.send_message(message.chat.id,"🆔 <b>Enter Server Product ID:</b>",parse_mode="HTML"); bot.register_next_step_handler(m,step_save_hack_server_product_id)
...
def step_save_hack_server_product_id(message):
    ...
    if not d.get('hack_server_product') or not pid:
        m=bot.send_message(message.chat.id,"❌ Product ID cannot be empty."); bot.register_next_step_handler(m,step_save_hack_server_product_id); return
...
def _ask_next_hack_valid_id(chat_id,uid):
    ...
    plan=q[i]; m=bot.send_message(chat_id,f"🆔 <b>Enter Valid ID for {html.escape(plan['days'])}:</b>\n\nPlan {i+1} of {len(q)}",parse_mode="HTML"); bot.register_next_step_handler(m,step_save_hack_valid_id)
...
def step_save_hack_valid_id(message):
    ...
    if not valid:
        m=bot.send_message(message.chat.id,"❌ Valid ID cannot be empty."); bot.register_next_step_handler(m,step_save_hack_valid_id); return
...
def handle_menu_click(call):
    ...
    # ROBUST WALLET PAYMENT CALLBACK
        ...
        if user_id in ADMIN_IDS:
            markup=types.InlineKeyboardMarkup()
            for title,cb in [("⚡ Setup & Product Management","admin_cat_setup"),("🌐 API & Server Systems","admin_cat_api"),("✨ Premium Emoji & Button UI","admin_cat_emoji"),("📝 Premium Text & Price List","admin_cat_text"),("👥 Users, Reseller & Discounts","admin_cat_users"),("💰 Balance & Payments","admin_cat_money"),("📢 Broadcast, Guide & Tracking","admin_cat_tools"),("⚙️ Bot / Database Tools","admin_cat_system")]:
                markup.add(types.InlineKeyboardButton(title,callback_data=cb))
            bot.send_message(call.message.chat.id,"🛠️ <b>ADMIN CONTROL PANEL</b>\n\nChoose a category below:",parse_mode="HTML",reply_markup=markup)

    elif call.data.startswith("admin_cat_"):
        if user_id in ADMIN_IDS:
            category=call.data.replace("admin_cat_",'',1); bot.send_message(call.message.chat.id,"🛠️ <b>ADMIN SECTION</b>\n\nSelect an operation:",parse_mode="HTML",reply_markup=_admin_category_markup(category))
        ...
        if user_id in ADMIN_IDS:
            ...
            if t_key == "welcome_message":
                ...
                bot.register_next_step_handler(msg,step_save_welcome_message); return
