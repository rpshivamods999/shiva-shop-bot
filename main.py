...
TOKEN = '8941581162:AAGr4V7IRIl5fGLqghdBSQHxUcCfj1yDwMY'
...
def _new_server_plan_valid_id(product_name, plan_id):
    conn = sqlite3.connect('shop_data.db', timeout=15)
    try:
        cur = conn.cursor(); cur.execute('SELECT valid_id FROM new_server_plan_ids WHERE product_name=? AND plan_id=?', (str(product_name), int(plan_id))); row = cur.fetchone(); return str(row[0]).strip() if row and row[0] is not None else ''
    finally: conn.close()
...
def fetch_key_from_api(pid_id, duration, android_id=None, product_name=None, plan_id=None):
    route=_resolve_api_route(product_name or '',plan_id,pid_id,duration)
    curr_api_url,curr_api_key,remote_product_id,valid_id=route['url'],route['api_key'],route['product_id'],route['valid_id']
    if not remote_product_id or remote_product_id=='0': remote_product_id='133'
    if not duration or duration=='0': duration='1 Hours'
    headers={'Content-Type':'application/x-www-form-urlencoded','x-master-key':RESELLER_MASTER_KEY}
    payload={'api_key':str(curr_api_key).strip(),'action':'buy','product_id':str(remote_product_id).strip(),'duration':str(duration).strip()}
    if valid_id: payload.update({'valid_id':str(valid_id).strip(),'validity_id':str(valid_id).strip()})
    if android_id: payload['android_id']=str(android_id).strip()
    try:
        response=HTTP_SESSION.post(curr_api_url,data=payload,headers=headers,timeout=15); raw_resp=response.text.strip(); print(f'DEBUG PROVIDER API RESPONSE: {raw_resp}')
        try:
            data=response.json()
            if isinstance(data,dict):
                ...
                if isinstance(data.get('data'),dict):
                    for field in ['key','license','serial','code','key_value','licence']:
                        if data['data'].get(field): return str(data['data'][field]).strip()
                if isinstance(data.get('data'),str) and len(data['data'])>3: return data['data'].strip()
        except Exception: pass
        if raw_resp and not raw_resp.startswith('{') and len(raw_resp)<120: return raw_resp
    except Exception as e: print(f'API Request Error: {e}')
    return None
...
def process_successful_payment(chat_id, user_id, message_id, order_id, amount, pay_type, utr, product_id=0, device_id=None):
    ...
    if pay_type == "topup":
        ...
        if not key_val:
            key_val = fetch_key_from_api(pid_id, remote_dur, android_id=device_id, product_name=prod_name, plan_id=product[0])
        if not key_val:
...
def handle_menu_click(call):
    ...
    # ROBUST WALLET PAYMENT CALLBACK
        ...
        if user_id in ADMIN_IDS:
            ok,detail=_new_server_test_connection(); set_setting(NEW_SERVER_ENABLED_KEY,'1' if ok else '0')
            text="🆕 <b>NEW SERVER API SYSTEM</b>\n\n"+("🟢 <b>Connected & Active</b>" if ok else "🟡 <b>Server configured; connection test failed</b>")+"\n\nSelect a hack to configure its New Server Product ID and plan-by-plan Valid IDs."
            bot.send_message(call.message.chat.id,text,parse_mode="HTML",reply_markup=_new_server_hacks_markup())
