...
TOKEN = '8941581162:AAF_S60KdGKI1UBo3agE5joWgTS9iK70lmM'
...
def _extract_api_key_value(data):
    """Extract a delivered key from common REST/JSON response shapes."""
    if isinstance(data, dict):
        direct_fields = (
            'key','license','license_key','licenseKey','licence','serial','code',
            'key_value','keyValue','access_key','accessKey','product_key',
            'productKey','generated_key','generatedKey','delivery_key','deliveryKey'
        )
        for field in direct_fields:
            ...
            if value not in (None,'') and not isinstance(value,(dict,list,bool)):
                ...
                if len(value)>3:
                    return value
        for field in ('data','result','response','order','delivery','sale','purchase','item','license_data','licenseData'):
...
def _new_api_get_order_key(server_url, api_key, order_id, max_wait_seconds=10):
    ...
    deadline=time.time()+max(1,int(max_wait_seconds or 10)); last_detail=''
    ...
    for payload in payloads:
        ...
        try:
            ...
            if response.status_code in (200,201,202,409) and order_id:
                ...
                key=_new_api_get_order_key(server_url,api_key,order_id,max_wait_seconds=10)
                if key: return key
